...
public class HandlerCommandDbcc extends HandlerCommandBase {

    static Logger logger = Logger.getLogger(HandlerCommandDbcc.class);

    public HandlerCommandDbcc() {
        super();
    }
    ...
    private SybaseVo getLineInfoForSybaseLog(Vector lines) {
        ...
        for (int i = 0; i < lines.size(); i++) {
            ...
            if (this.isIncludeKeyInLine(FrameDBConstant.Command_find_dbcc_key_space_used, line)) {
                spaceUsed = this.getLineField(line, 1, FrameDBConstant.Command_seperator_comma);
                spaceUsed = this.getSpace(spaceUsed);
                vo.setUsedLog(spaceUsed);
                continue;
            }
            if (this.isIncludeKeyInLine(FrameDBConstant.Command_find_dbcc_key_space_free, line)) {
                spaceFree = this.getLineField(line, 1, FrameDBConstant.Command_seperator_comma);
                spaceFree = this.getSpace(spaceFree);
