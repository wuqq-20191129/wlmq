 ...
 * @author goldsign
 */
public class TestSocket57 extends Thread{
...
//    public static final String ipAddress = "10.99.11.81";
    ...
    public static void main(String[] args) {
        TestSocket57 myThread;
        myThread = new TestSocket57();
        myThread.run();
    }
        ...
        try {
            ...
            for (int i=0; i<5; i++) {
                ...
                if(i==0){
                    ...
                    vo.setTerminalNo("010101131");
                    vo.setAccSeq(11L);
                    vo.setResultCode("00");
                    vo.setOrderNo("20181207000001");
                    vo.setPhoneNo("13800000000");
                    vo.setSaleFee(200L);
                    vo.setSaleTimes(1L);
                    vo.setDealFee(400L);
                    vo.setDealTime("20181207000001");
...
//                }else if(i==2){
                ...
                if(dataType==3){
                    ...
                    sb.append(","+getCharString(data, 42, 2));//响应码
                }
                sb.append(","+in.read());
...
//                    socket.close();
    ...
    private byte[] data(QrCodeVo vo){
        byte[] b = new byte[81];
        System.arraycopy("57".getBytes(), 0, b, 0, 2);//消息类型
        ...
        System.arraycopy(longStringToByteArray(vo.getAccSeq(),4), 0, b, 38, 4);//终端处理流水号
        System.arraycopy(vo.getResultCode().getBytes(), 0, b, 42, 2);//执行结果
        System.arraycopy(vo.getOrderNo().getBytes(), 0, b, 44, 14);//订单号
        System.arraycopy(vo.getPhoneNo().getBytes(), 0, b, 58, 11);//手机号
        System.arraycopy(longStringToByteArray(vo.getSaleFee(),4), 0, b, 69, 4);//
        System.arraycopy(longStringToByteArray(vo.getSaleTimes(),4), 0, b, 73, 4);//
        System.arraycopy(longStringToByteArray(vo.getDealFee(),4), 0, b, 77, 4);//
//        System.arraycopy(bcdStringToByteArray(vo.getDealTime()), 0, b, 81, 7);//取票时间
        return b;
    }
