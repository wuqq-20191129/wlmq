        ...
        if (avriableOpertions.indexOf(",") != -1) {
            while (index_n != -1) {
                ...
                if (index_n > 0) {
                    ...
                    var buttonName = avriableOpertions.substring(index_p, index_n - 1);
                    //alert(buttonName);
                    if (buttonName != "btNext" && buttonName != "btNextEnd" && buttonName != "btBack" && buttonName != "btBackEnd"&& buttonName != "btGoPage") {
                        disableButton(formName, buttonName, flag);
                    }
                    index_p = index_n;
                }
...
function clickOneResultRow(formName, object, detailFrom, name) {
    ...
    var sameLineFlag = false;
    if (oldLine == headEventObject.id) {
        sameLineFlag = true;
    }
    if (sameLineFlag) {
        headEventObject.style.background = rowBackGround;
        setCurrentLine(formName, "init");
        setOperation(formName, "op_browse");
//      setCurrentLine(formName, headEventObject.id);
    } else {
        //设置当前行
        ...
        //改变背景色
        rowBackGround = headEventObject.style.backgroundColor;
        headEventObject.style.background = "#97CBFF";//票务绿 运营蓝#97CBFF
    }
    if (headEventObject.children.length > 0) {
        headEventObject = headEventObject.children[0].children[0];
        if (headEventObject.type == "checkbox" && headEventObject.name == name) {
            if (!sameLineFlag) {
                //如果没够被勾上
                if (!headEventObject.checked) {
...
//                  alert("2"+headEventObject.checked);
                } else if (checkBoxFlag == 1) {
//            	  alert("1"+headEventObject.checked);
                    temp++;
                }
            } else {
...
//          alert(count.getAttribute('value'));
    ...
    //改变操作模式（改变按钮的可用状态）
//  alert("getCurrentLine(formName):"+getCurrentLine(formName));
    setDefaultOperation(formName);
    //填充明细框内容
    if (sameLineFlag) {
        setDetailFormContentsBlank(formName, detailFrom);
    } else {
        setDetailFormContents(formName, detailFrom);
    }
}
//add by zhongzq 20180425
function setDefaultOperation(formName) {
    if (document.forms[formName] == undefined) {
        return;
    }
    var opertion = document.forms[formName].getElementsByTagName("input")["operation"];
    //alert("opertion.value:"+opertion.value);
    if (opertion.value !== "op_select") {  //如果在op_select模式下则不用改变，否则改变为op_select_browse
        //改变操作模式（改变按钮的可用状态）
        setOperation(formName, "op_browse");
    } else
...
//add by zhongzq 20180508 清理内容
function setDetailFormContentsBlank(formName, detailForm) {
    if (detailForm != "") {
        var detail = document.getElementById(detailForm);
//        	alert(detail);
        if (detail != null) {
            var detailTable = detail.children[0];//table
            if (detailTable == undefined) {
                return;
            }
            while (detailTable.children[0].tagName != "TR") {
                detailTable = detailTable.children[0];
            ...
            var currentLine = getCurrentLine(formName);
//            alert(currentLine);
            if (currentLine == "" || currentLine == "init") {
                headEventObject = event.srcElement;
                while (headEventObject.tagName != "TR") {
                    headEventObject = headEventObject.parentElement;
                }
//            	alert("headEventObject.id:"+headEventObject.id);
                var currentObject = document.getElementById(headEventObject.id);
                var tmp = currentObject.children.length;
//                alert(tmp);
                if (tmp != 0) {
                    for (var i = 0; i < tmp; i++) {
...
//add by zhongzq 20180508 全选按钮代替selectAllRecord
function selectAll(formName, headCheckName, name, listDivName, colNo) {
    var currentOperation = getCurrentOpertion(formName);
    if (currentOperation == "op_modify" || currentOperation == "op_add")
        return false;
    var obj = document.getElementById(headCheckName);
    ...
    for (var i = 0; i < tbl.rows.length; i++)
    {
        ...
        if (chk.name == name && chk.type == "checkbox")
        {
            if (obj.checked) {
            	...
            	var curlineId = getCurrentLine(formName);
            	if(curlineId!=''||curlineId!='init'){
//            		alert("tbl.rows[i].id:"+tbl.rows[i].id+"||"+"curlineId:"+curlineId+"||tbl.rows[i].id==curlineId:");
            		if(tbl.rows[i].id==curlineId){
            			chk.checked = "1";
            			temp++;
            			clickRowFlag= true;
            		}else{
