...
public class AppUtil {
    ...
    public static String[] HourLineTitles = null;
    public static String[] HourLineTitlesFormat = null;
    ...
    DecimalFormat df = new DecimalFormat("#.00");//数据格式化：四舍五入，保留两位有效小数
    /**
     * 取配置文件线路颜色值
     * @param common 
     */
     ...
     * @param common 
     */
    private void getColor5MinLine(Hashtable common) {
        String colorLines = (String) common.get(XMLSAXHandler.Common_ColorLine5Min_TAG);
        StringTokenizer st = new StringTokenizer(colorLines,"#");
        String token;
        int index;
        String flag;
        ...
        while(st.hasMoreTokens()){
            token = st.nextToken();
            index = token.indexOf(",");
            flag = token.substring(0,index);
            color = token.substring(index+1); 
            rgb =this.getColorRGB(color);
            DrawUtil.COLOR_5MinLINES.put(new Integer(flag), new Color(rgb[0],rgb[1],rgb[2]));
        }
    }
    ...
    public void getInitConfiguration() throws Exception {
        ...
        //数据源3
        String driver3 = (String) AppConstant.dbConfig.get(XMLSAXHandler.ConnectionPool_Driver_TAG3);
        String url3 = (String) AppConstant.dbConfig.get(XMLSAXHandler.ConnectionPool_URL_TAG3);
        String userId3 = (String) AppConstant.dbConfig.get(XMLSAXHandler.ConnectionPool_User_TAG3);
        String password3 = (String) AppConstant.dbConfig.get(XMLSAXHandler.ConnectionPool_Password_TAG3);
        ...
        AppConstant.dbcpHelper3 = new DbcpHelper(driver3, url3, userId3, password3, maxActive, maxIdle, maxWait);
    ...
    public JFreeChart getPicForSpanHourLineForLocal(Vector drV) {
        return this.drawLineScreenForLocal("",
                AppConstant.SCREEN_CAPTION_ROW_HOUR,
                AppConstant.SCREEN_CAPTION_COLUMN_PERSON, drV);
    }
...
/*
    ...
    public void addTableForCard(Vector dataSub, JScrollPane jScrollPannel){
        ...
        for (i = 0; i < colNum; i++) {
            ...
            columnNames.add("进站(人次/百分比)");
            columnNames.add("出站(人次/百分比)");
        }
        Vector dataV = new Vector();
    ...
    private Vector setTableDataCard(Map<String, Vector> map) {
        ...
        if(dataSub.size()>0){
            dataSorted = SelectedSortViewvos(dataSub);
            return dataSorted;
        }
     ...
     * @return 
     */
    public static Vector SelectedSortViewvos(Vector dataVec){
        
        ViewVo vvo = new ViewVo();
        ViewVo vvt = new ViewVo();
        String voCardId = "";
        String vtCardId = "";
        Vector dataSorted = new Vector();
        while(dataVec.size()>1){
            ...
            for(int i=1;i<dataVec.size();i++){
                vvo = (ViewVo)dataVec.get(minIndex);
                voCardId = vvo.getMainCardID() + vvo.getSubCardID();
                vvt = (ViewVo)dataVec.get(i);
                vtCardId = vvt.getMainCardID() + vvt.getSubCardID();
                if(voCardId.compareTo(vtCardId) > 0){//前面的数字大于后面的数字就交换
                    minIndex = i;
                }
            }
            dataSorted.add(dataVec.get(minIndex));
            dataVec.remove(minIndex);
        }
        dataSorted.add(dataVec.get(0));
        return dataSorted;
    }
    ...
    private void getTableDataForCardFormated(Vector dataSub, Vector dataV, int num) {
        ...
        for(int i=0; i<dataSub.size(); i++){
            ...
            String subInPercent = totalIn==0?"0":getFormatPercentIn(vo, totalIn);
            String subOutPercent = totalIn==0?"0":getFormatPercentOut(vo, totalOut);
            dataT.add(this.formatInteger(vo.getTraffic_in()) + " ("+subInPercent+")");
            dataT.add(this.formatInteger(vo.getTraffic_out()) + " ("+subOutPercent+")");
        }
        dataT.add("[总计]");
        dataT.add("[" + this.formatInteger(String.valueOf(totalIn)) + "]");
        dataT.add("[" + this.formatInteger(String.valueOf(totalOut)) + "]");
     ...
     * @return 
     */
    private String getFormatPercentIn(ViewVo vo, int totalIn){
        String formatStr = "";
        double per = Integer.parseInt(vo.getTraffic_in())*100.0/totalIn;
        if(per<1.0){
            formatStr = "0"+df.format((Integer.parseInt(vo.getTraffic_in())*100.0/totalIn))+"%";
        }else{
            formatStr = df.format((Integer.parseInt(vo.getTraffic_in())*100.0/totalIn))+"%";
        }
        return formatStr;
    }
    /**
     ...
     * @return 
     */
    private String getFormatPercentOut(ViewVo vo, int totalOut){
        String formatStr = "";
        double per = Integer.parseInt(vo.getTraffic_out())*100.0/totalOut;
        if(per<1.0){
            formatStr = "0"+df.format((Integer.parseInt(vo.getTraffic_out())*100.0/totalOut))+"%";
        }else{
            formatStr = df.format((Integer.parseInt(vo.getTraffic_out())*100.0/totalOut))+"%";
        }
        return formatStr;
    }
...
//        jScrollPannel.removeAll();
    ...
    //小时客流(折线图)模式
    public boolean isModeForHourLine() {
        if (AppConstant.CURRENT_MODE == AppConstant.MODE_SHOW_HOUR_LINE) {
            return true;
        }
        return false;
    }
    ...
    public void processFlow(JTree tree, JPanel jPanelGraph, JScrollPane jScrollPane1Data,
        ...
        synchronized (AppUtil.synVo) {
            ...
            try {
                ...
                //处理小时客流(折线图)
                if (util.isModeForHourLine()) {
                    this.processHourLine(treeNode, jPanelGraph, jScrollPane1Data,
                            jLabelDataCaption, jLabelGraphCaption);
                }
    ...
    void processCard(TreeNode treeNode, JPanel jPanelGraph,JScrollPane jScrollPane1Data, JLabel jLabelDataCaption) throws Exception {
        ...
        long endTime;
        //修改title文字
        jLabelDataCaption.setText(this.getCaptionDataForMin(treeNode) + "（票卡）");
        //数据库取票卡类型客流数据
    ...
    //小时客流（折线图）模式
    private void processHourLine(TreeNode treeNode, JPanel jPanelGraph, JScrollPane jScrollPane1Data,
            JLabel jLabelDataCaption, JLabel jLabelGraphCaption) throws Exception {
        String currentDate = DateHelper.datetimeToString(new Date());// "2009-06-21 14:00:00";
        String lineId = null;
        int index = -1;
        ...
        JFreeChart chart = util.getPicForSpanHourLineForLocal(traffics);
        util.addChart(jPanelGraph, chart);
