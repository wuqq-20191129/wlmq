/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package com.goldsign.acc.app.storageout.entity;

/**
 *
 * @author hejj
 */
public class TicketStorageOutProducePlanDetail {

    /**
     * @return the valid_date
     */
    public String getValid_date() {
        return valid_date;
    }

    /**
     * @param valid_date the valid_date to set
     */
    public void setValid_date(String valid_date) {
        this.valid_date = valid_date;
    }

    /**
     * @return the es_worktype_id_name
     */
    public String getEs_worktype_id_name() {
        return es_worktype_id_name;
    }

    /**
     * @param es_worktype_id_name the es_worktype_id_name to set
     */
    public void setEs_worktype_id_name(String es_worktype_id_name) {
        this.es_worktype_id_name = es_worktype_id_name;
    }

    /**
     * @return the storage_id_name
     */
    public String getStorage_id_name() {
        return storage_id_name;
    }

    /**
     * @param storage_id_name the storage_id_name to set
     */
    public void setStorage_id_name(String storage_id_name) {
        this.storage_id_name = storage_id_name;
    }

    /**
     * @return the area_id_name
     */
    public String getArea_id_name() {
        return area_id_name;
    }

    /**
     * @param area_id_name the area_id_name to set
     */
    public void setArea_id_name(String area_id_name) {
        this.area_id_name = area_id_name;
    }

    /**
     * @return the ic_main_type_name
     */
    public String getIc_main_type_name() {
        return ic_main_type_name;
    }

    /**
     * @param ic_main_type_name the ic_main_type_name to set
     */
    public void setIc_main_type_name(String ic_main_type_name) {
        this.ic_main_type_name = ic_main_type_name;
    }

    /**
     * @return the ic_sub_type_name
     */
    public String getIc_sub_type_name() {
        return ic_sub_type_name;
    }

    /**
     * @param ic_sub_type_name the ic_sub_type_name to set
     */
    public void setIc_sub_type_name(String ic_sub_type_name) {
        this.ic_sub_type_name = ic_sub_type_name;
    }

    /**
     * @return the line_id_name
     */
    public String getLine_id_name() {
        return line_id_name;
    }

    /**
     * @param line_id_name the line_id_name to set
     */
    public void setLine_id_name(String line_id_name) {
        this.line_id_name = line_id_name;
    }

    /**
     * @return the station_id_name
     */
    public String getStation_id_name() {
        return station_id_name;
    }

    /**
     * @param station_id_name the station_id_name to set
     */
    public void setStation_id_name(String station_id_name) {
        this.station_id_name = station_id_name;
    }

    /**
     * @return the card_type_name
     */
    public String getCard_type_name() {
        return card_type_name;
    }

    /**
     * @param card_type_name the card_type_name to set
     */
    public void setCard_type_name(String card_type_name) {
        this.card_type_name = card_type_name;
    }

    /**
     * @return the reason_id_name
     */
    public String getReason_id_name() {
        return reason_id_name;
    }

    /**
     * @param reason_id_name the reason_id_name to set
     */
    public void setReason_id_name(String reason_id_name) {
        this.reason_id_name = reason_id_name;
    }

    /**
     * @return the exit_line_id_name
     */
    public String getExit_line_id_name() {
        return exit_line_id_name;
    }

    /**
     * @param exit_line_id_name the exit_line_id_name to set
     */
    public void setExit_line_id_name(String exit_line_id_name) {
        this.exit_line_id_name = exit_line_id_name;
    }

    /**
     * @return the exit_station_id_name
     */
    public String getExit_station_id_name() {
        return exit_station_id_name;
    }

    /**
     * @param exit_station_id_name the exit_station_id_name to set
     */
    public void setExit_station_id_name(String exit_station_id_name) {
        this.exit_station_id_name = exit_station_id_name;
    }

    /**
     * @return the model_name
     */
    public String getModel_name() {
        return model_name;
    }

    /**
     * @param model_name the model_name to set
     */
    public void setModel_name(String model_name) {
        this.model_name = model_name;
    }

    /**
     * @return the sale_flag_name
     */
    public String getSale_flag_name() {
        return sale_flag_name;
    }

    /**
     * @param sale_flag_name the sale_flag_name to set
     */
    public void setSale_flag_name(String sale_flag_name) {
        this.sale_flag_name = sale_flag_name;
    }

    /**
     * @return the test_flag_name
     */
    public String getTest_flag_name() {
        return test_flag_name;
    }

    /**
     * @param test_flag_name the test_flag_name to set
     */
    public void setTest_flag_name(String test_flag_name) {
        this.test_flag_name = test_flag_name;
    }

    /**
     * @return the order_type_name
     */
    public String getOrder_type_name() {
        return order_type_name;
    }

    /**
     * @param order_type_name the order_type_name to set
     */
    public void setOrder_type_name(String order_type_name) {
        this.order_type_name = order_type_name;
    }

    /**
     * @return the water_no
     */
    public int getWater_no() {
        return water_no;
    }

    /**
     * @param water_no the water_no to set
     */
    public void setWater_no(int water_no) {
        this.water_no = water_no;
    }

    /**
     * @return the bill_no
     */
    public String getBill_no() {
        return bill_no;
    }

    /**
     * @param bill_no the bill_no to set
     */
    public void setBill_no(String bill_no) {
        this.bill_no = bill_no;
    }

    /**
     * @return the es_worktype_id
     */
    public String getEs_worktype_id() {
        return es_worktype_id;
    }

    /**
     * @param es_worktype_id the es_worktype_id to set
     */
    public void setEs_worktype_id(String es_worktype_id) {
        this.es_worktype_id = es_worktype_id;
    }

    /**
     * @return the storage_id
     */
    public String getStorage_id() {
        return storage_id;
    }

    /**
     * @param storage_id the storage_id to set
     */
    public void setStorage_id(String storage_id) {
        this.storage_id = storage_id;
    }

    /**
     * @return the area_id
     */
    public String getArea_id() {
        return area_id;
    }

    /**
     * @param area_id the area_id to set
     */
    public void setArea_id(String area_id) {
        this.area_id = area_id;
    }

    /**
     * @return the ic_main_type
     */
    public String getIc_main_type() {
        return ic_main_type;
    }

    /**
     * @param ic_main_type the ic_main_type to set
     */
    public void setIc_main_type(String ic_main_type) {
        this.ic_main_type = ic_main_type;
    }

    /**
     * @return the ic_sub_type
     */
    public String getIc_sub_type() {
        return ic_sub_type;
    }

    /**
     * @param ic_sub_type the ic_sub_type to set
     */
    public void setIc_sub_type(String ic_sub_type) {
        this.ic_sub_type = ic_sub_type;
    }

    /**
     * @return the card_money
     */
    public int getCard_money() {
        return card_money;
    }

    /**
     * @param card_money the card_money to set
     */
    public void setCard_money(int card_money) {
        this.card_money = card_money;
    }



    /**
     * @return the line_id
     */
    public String getLine_id() {
        return line_id;
    }

    /**
     * @param line_id the line_id to set
     */
    public void setLine_id(String line_id) {
        this.line_id = line_id;
    }

    /**
     * @return the station_id
     */
    public String getStation_id() {
        return station_id;
    }

    /**
     * @param station_id the station_id to set
     */
    public void setStation_id(String station_id) {
        this.station_id = station_id;
    }

    /**
     * @return the draw_quantity
     */
    public int getDraw_quantity() {
        return draw_quantity;
    }

    /**
     * @param draw_quantity the draw_quantity to set
     */
    public void setDraw_quantity(int draw_quantity) {
        this.draw_quantity = draw_quantity;
    }

    /**
     * @return the make_quantity
     */
    public int getMake_quantity() {
        return make_quantity;
    }

    /**
     * @param make_quantity the make_quantity to set
     */
    public void setMake_quantity(int make_quantity) {
        this.make_quantity = make_quantity;
    }

    /**
     * @return the machine_no
     */
    public String getMachine_no() {
        return machine_no;
    }

    /**
     * @param machine_no the machine_no to set
     */
    public void setMachine_no(String machine_no) {
        this.machine_no = machine_no;
    }

    /**
     * @return the card_type
     */
    public String getCard_type() {
        return card_type;
    }

    /**
     * @param card_type the card_type to set
     */
    public void setCard_type(String card_type) {
        this.card_type = card_type;
    }

    /**
     * @return the reason_id
     */
    public String getReason_id() {
        return reason_id;
    }

    /**
     * @param reason_id the reason_id to set
     */
    public void setReason_id(String reason_id) {
        this.reason_id = reason_id;
    }

    /**
     * @return the card_money_produce
     */
    public int getCard_money_produce() {
        return card_money_produce;
    }

    /**
     * @param card_money_produce the card_money_produce to set
     */
    public void setCard_money_produce(int card_money_produce) {
        this.card_money_produce = card_money_produce;
    }

    /**
     * @return the card_ava_days
     */
    public String getCard_ava_days() {
        return card_ava_days;
    }

    /**
     * @param card_ava_days the card_ava_days to set
     */
    public void setCard_ava_days(String card_ava_days) {
        this.card_ava_days = card_ava_days;
    }

    /**
     * @return the exit_line_id
     */
    public String getExit_line_id() {
        return exit_line_id;
    }

    /**
     * @param exit_line_id the exit_line_id to set
     */
    public void setExit_line_id(String exit_line_id) {
        this.exit_line_id = exit_line_id;
    }

    /**
     * @return the exit_station_id
     */
    public String getExit_station_id() {
        return exit_station_id;
    }

    /**
     * @param exit_station_id the exit_station_id to set
     */
    public void setExit_station_id(String exit_station_id) {
        this.exit_station_id = exit_station_id;
    }

    /**
     * @return the model
     */
    public String getModel() {
        return model;
    }

    /**
     * @param model the model to set
     */
    public void setModel(String model) {
        this.model = model;
    }

    /**
     * @return the sale_flag
     */
    public String getSale_flag() {
        return sale_flag;
    }

    /**
     * @param sale_flag the sale_flag to set
     */
    public void setSale_flag(String sale_flag) {
        this.sale_flag = sale_flag;
    }

    /**
     * @return the test_flag
     */
    public String getTest_flag() {
        return test_flag;
    }

    /**
     * @param test_flag the test_flag to set
     */
    public void setTest_flag(String test_flag) {
        this.test_flag = test_flag;
    }

    /**
     * @return the order_type
     */
    public String getOrder_type() {
        return order_type;
    }

    /**
     * @param order_type the order_type to set
     */
    public void setOrder_type(String order_type) {
        this.order_type = order_type;
    }
    private int water_no;
    private String bill_no;
    private String es_worktype_id;
    private String es_worktype_id_name;
    private String storage_id;
    private String storage_id_name;
    private String area_id;
    private String area_id_name;
    private String ic_main_type;
    private String ic_main_type_name;
    private String ic_sub_type;
    private String ic_sub_type_name;
    private int card_money;
    private String valid_date;
    private String line_id;
    private String line_id_name;
    private String station_id;
    private String station_id_name;
    private int draw_quantity;
    private int make_quantity;
    private String machine_no;
    private String card_type;
    private String card_type_name;
    private String reason_id;
    private String reason_id_name;
    private int card_money_produce;
    private String card_ava_days;
    private String exit_line_id;
    private String exit_line_id_name;
    private String exit_station_id;
    private String exit_station_id_name;
    private String model;
    private String model_name;
    private String sale_flag;
    private String sale_flag_name;
    private String test_flag;
    private String test_flag_name;
    private String order_type;
    private String order_type_name;

}
