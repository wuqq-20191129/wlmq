 ...
 * @author zhangjh
 */
public class HanderTBH extends HanderTk {
	private static final Logger logger = Logger.getLogger(HanderTBH.class
			.getName());
	...
	public void handleMessage(HandleMessageBase msg) {
		...
		logger.info("收益组上交数据文件解析处理开始，文件名：" + fileName);
		doParse(msg, fileName);
		logger.info("收益组上交数据文件解析处理结束，文件名：" + fileName);
	}
}
