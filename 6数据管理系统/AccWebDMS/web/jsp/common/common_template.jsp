...
<c:choose>
    ...
    <c:when test="${templateName == 'option_list_pub_tableType'}">
        <!-- 源表类型 add by luck 20170914 -->
        <option value="">=请选择=</option>
        <c:forEach items="${tableTypes}" var="tableType"> 
            <c:choose>
                <c:when test="${tableType.code_text == ''}">
                    <option value="${tableType.code}">${tableType.code}</option>
                </c:when>
                <c:otherwise>
                    <option value="${tableType.code}">${tableType.code_text}</option>
                </c:otherwise>
            </c:choose>
        </c:forEach>
    </c:when>
