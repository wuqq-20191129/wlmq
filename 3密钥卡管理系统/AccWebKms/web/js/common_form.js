                                        ...
                                        switch (tmpObj.type) {
                                            ...
                                            case "text":
                                            {
                                                ...
                                                if (child.children[0] != null && child.children[0].tagName == 'A') {
                                                    tmpObj.value = child.children[0].innerText.trim();
                                                }
                                        ...
                                        if (tmpObj.type == "select-one") {
                                            if (tmpObj.options.length < 1) {
                                                ...
                                                for (j = 0; j < tmpObj.options.length; j++) {
                                                    if (tmpObj.options[j].value == child.innerHTML.trim() || tmpObj.options[j].text == child.innerHTML.trim()) {
                                                        tmpObj.options[j].selected = "true";
                                                        break;
                                                    }
                                                    if (j == tmpObj.options.length - 1 && tmpObj.options[0].id == "produceOrderSelf") {
                                                        tmpObj.options[0].selected = "true";
                                                        tmpObj.options[0].value = child.innerHTML.trim();
                                                        tmpObj.options[0].text = child.innerHTML.trim();
                                                    }
                                                }
                                            }
                                        }
...
//modify by hejj 20110528
        ...
        setDetailFormEnabledBySelName(formName, true);
...
// alert('i='+i+' j='+j+' '+cellText);
            ...
            oSheet.Cells(l + 1, k + 1).Font.Size = 10; //设置为10号字*
            oSheet.Columns.AutoFit(); //自动列宽
            if (i == 0 &&indexStartLine==0) { //第一行不加粗
                oSheet.Cells(l + 1, k + 1).Font.Bold = true; //设置为粗体*
            }
            k++;
        }
...
function setDetailFormEnabledBySelName(formName, flag) {
    var detail = document.forms[formName];
    var selects = detail.getElementsByTagName("select");
    if (detail != null) {
        for (i = 0; i < selects.length; i++) {
