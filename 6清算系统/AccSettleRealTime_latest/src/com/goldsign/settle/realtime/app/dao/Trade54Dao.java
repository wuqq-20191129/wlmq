 ...
 * @author hejj
 */
public class Trade54Dao  extends TradeBaseDao{
    ...
    @Override
     public int insertError(FileRecordBase frb,String errCode)  {
        ...
        String sql = "insert into "+FrameDBConstant.DB_PRE+"ST_ERR_DEAL(LINE_ID,STATION_ID,DEV_TYPE_ID,DEVICE_ID,SAM_LOGICAL_ID,"
                                             ...
                                             + "ACCUMULATE_CONSUME_FEE,INTERVAL_BETWEEN_BUS_METRO,LAST_BUS_DEAL_DATETIME,ACCUMULATE_CONSUME_NUM,TCT_ACTIVATE_DATETIME "
                                             + ") "
                                             + "values(?,?,?,?,?,"
                                             + "?,?,?,?,?,"
                                             + "?,?,?,?,?,"
                                             ...
                                             + "?,?,?,?,?,"
                                             + "?,?,?,?,?,"
                                             + "?,W_ACC_ST.W_SEQ_W_ST_ERR_DEAL.nextval,?,?,?,"
                                             + "?,?,?,?,?,"
                                             + "?,?,?,?,?,"
                                             + "?,?,?,?,?,"
                                             +"?,?,?,?,?,"
        ...
        Object[] values = {vo.getLineId(),vo.getStationId(),vo.getDevTypeId(),vo.getDeviceId(),vo.getSamLogicalId(),
                           ...
                           vo.getAccumulateConsumeFee_s(),vo.getIntervalBetweenBusMetro_s(),vo.getLastBusDealDatetime_s(),vo.getAccumulateConsumeNum_s(),vo.getTctActiveDatetime_s()
                          };
        try {
