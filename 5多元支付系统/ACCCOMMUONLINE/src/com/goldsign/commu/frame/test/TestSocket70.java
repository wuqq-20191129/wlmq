 ...
 * @author goldsign
 */
public class TestSocket70 extends Thread{
...
//    public static final String ipAddress = "210.21.94.85";
    ...
    public static void main(String[] args) {
        TestSocket70 myThread;
        myThread = new TestSocket70();
        myThread.run();
    }
    ...
    public void run(){
        ...
        try {
            ...
            String orderNo = DateHelper.currentTodToString();
            ...
            for (int i=0; i<5; i++) {
                ...
                if(i==0){
                    ...
                    vo.setTerminaSeq(1L);
//                    vo.setSamLogicalId("83000003A0000565");//外部
                    vo.setSamLogicalId("83000003A0000566");//127.0.0.1
                    vo.setTerminalNo("011010132");
                    vo.setOrderNo(orderNo);
                    vo.setOrderDate(orderNo);
                    vo.setCardType("0100");
                    vo.setSaleFee(200L);
                    vo.setSaleTimes(2L);
                    vo.setDealFee(400L);
...
//                }else if(i==2){
                ...
                if(dataType==3){
                    ...
                    sb.append(","+getCharString(data, 18, 16));//PSAM逻辑卡号
                    sb.append(","+getLong(data, 34));//终端流水号
                    sb.append(","+getLong(data, 38));//中心流水号
                    sb.append(","+getCharString(data, 42, 14));//订单号
                    sb.append(","+getCharString(data, 56, 10));//支付标识
                    sb.append(","+getCharString(data, 66, 34));//支付二维码信息
                    sb.append(","+getCharString(data, 100, 2));//响应码
                }
                sb.append(","+in.read());
...
//                    socket.close();
    ...
    private byte[] data(QrPayOrderVo vo){
        byte[] b = new byte[73];
        System.arraycopy("70".getBytes(), 0, b, 0, 2);//消息类型
        ...
        System.arraycopy(longStringToByteArray(vo.getTerminaSeq(),4), 0, b, 34, 4);//终端流水号
        System.arraycopy(vo.getOrderNo().getBytes(), 0, b, 38, 14);//订单号
        System.arraycopy(bcdStringToByteArray(vo.getOrderDate()), 0, b, 52, 7);//订单生成时间
        System.arraycopy(bcdStringToByteArray(vo.getCardType()), 0, b, 59, 2);//票卡类型
        System.arraycopy(longStringToByteArray(vo.getSaleFee(),4), 0, b, 61, 4);//发售单程票单价
        System.arraycopy(longStringToByteArray(vo.getSaleTimes(),4), 0, b, 65, 4);//发售单程票数量
        System.arraycopy(longStringToByteArray(vo.getDealFee(),4), 0, b, 69, 4);//发售单程票总价
        return b;
    }
