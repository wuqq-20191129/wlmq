 ...
 * @author hejj
 */
public class Trade57Dao extends TradeBaseDao{
    private static Logger logger = Logger.getLogger(Trade57Dao.class.getName());
    ...
    public int insertError(FileRecordBase frb,String errCode) throws Exception {
         FileRecord57 vo = (FileRecord57) frb;

        String sql = "insert into W_ACC_ST.W_ST_ERR_RETURN(LINE_ID,STATION_ID,DEV_TYPE_ID,DEVICE_ID,SAM_LOGICAL_ID,"
                                             ...
                                             + "TAC_DEAL_TYPE,TAC_DEV_ID,BALANCE_WATER_NO_SUB,mobile_no,paid_channel_type,"
                                             + "paid_channel_code,RECORD_VER"                                            
                                             + ") "
                                             + "values("
                                             + "?,?,?,?,?,"
                                             + "?,?,?,?,?,"
                                             ...
                                             + "?,?,?,?,?,"
                                             + "?,?,?,?,?,"
                                             + "?,?,?,?,W_ACC_ST.W_SEQ_W_ST_ERR_RETURN.nextval,"
                                             + "?,?,?,?,?"
                                             + "?,?"                                           
                                              + ") ";
        ...
        Object[] values = {vo.getLineId(),vo.getStationId(),vo.getDevTypeId(),vo.getDeviceId(),vo.getSamLogicalId(),
                           ...
                           vo.getBusTacDealType(),vo.getBusTacDevId(),vo.getBalanceWaterNoSub(),vo.getMobileNo(),vo.getPaidChannelType(),
                           vo.getPaidChannelCode(),vo.getRecordVer()
