...
begin
  ---------------------------------基本信息处理-------------------------------------------------------------
  begin
    delete w_acc_st.w_t#w_st_results_non_rtn;
  ...
  begin
    if (isNeedBaseinfo = 1) then
      begin
        insert into w_acc_st.w_t#w_st_results_non_rtn
          (card_logical_id,
           deposit_fee,
           card_balance_fee,
           actual_return_bala,
      ...
      select count(card_logical_id)
        into v_temp_counts
        from w_acc_st.w_t#w_st_results_non_rtn;
      if (v_temp_counts = 1) then
        ...
        if (audt_flag is null or audt_flag <> '1' or
          ...
          update w_acc_st.w_t#w_st_results_non_rtn
    ...
    if (i_applyaction = actionQuery) then
      ...
      insert into w_acc_st.w_st_flow_non_rtn
        ...
        select w_acc_st.w_seq_w_st_flow_non_rtn.nextval,
               ...
               hdl_flag,
               sysdate
          from w_acc_st.w_t#w_st_results_non_rtn; ----返回处理信息
  ...
  begin
    if (i_applyaction = actionFinish) then
      begin
        insert into w_acc_st.w_t#W_ST_FLAG_NON_RTN
          (return_flag)
          select return_flag
            from w_acc_st.w_st_flow_non_rtn
           where id = i_id
        ...
        select count(*)
          into v_temp_counts1
          from w_acc_st.w_t#W_ST_FLAG_NON_RTN
