 ...
 * @author goldsign
 */
public class TestSocket36 extends Thread{
...
//    public static final String ipAddress = "127.0.0.1";
    ...
    public static void main(String[] args) {
        TestSocket36 myThread;
        myThread = new TestSocket36();
        myThread.run();
    }
...
//                }else if(i==2){
                ...
                if(dataType==3){
                    ...
                    sb.append(","+getCharString(data, 42, 64));//二维码
                    sb.append(","+getCharString(data, 106, 14));//订单号
                    sb.append(","+getCharString(data, 120, 11));//手机号
                    sb.append(","+getBcdString(data, 131, 7));//有效时间
                    sb.append(","+getCharString(data, 138, 2));//响应码
                    sb.append(","+getCharString(data, 140, 2));//错误码
                }
                sb.append(","+in.read());
...
//                    socket.close();
    ...
    private byte[] data(QrCodeVo vo){
        byte[] b = new byte[75];
        System.arraycopy("36".getBytes(), 0, b, 0, 2);//消息类型
        ...
        System.arraycopy(vo.getOrderNo().getBytes(), 0, b, 34, 14);//订单号
        System.arraycopy(longStringToByteArray(vo.getTerminaSeq(),4), 0, b, 48, 4);//终端处理流水号
