<%-- 
    Document   : TicketStorageInCancelOutBill
...
<html>
    ...
    <body onload="initDocument('queryOp', 'detail');
        ...
        <form method="post" name="queryOp" id="queryOp" action="ticketStorageInCheckManage">
            <!-- 页面用到的变量 通用模板 -->
                    ...
                    <c:forEach items="${ResultSet}" var="rs">
                        ...
                        <tr class="table_list_tr_data"  onMouseOver="overResultRow('detailOp', this);" 
                            ...
                            onclick="controlsByFlagAndMul('detailOp', ['audit']);
                                    ...
                                    controlsDisableForMulRecords('detailOp', ['del']);
                                    setPageControl('detailOp');" 
                            id="${rs.bill_no}#${rs.related_bill_no}" flag="${rs.record_flag}">
                            ...
                            <td  id="billNo" class="table_list_tr_col_data_block" style="width: 90px">
                                <a href='#'
                                   onClick="openwindow('ticketStorageInCheckManage?queryCondition=${rs.bill_no}&command=queryDetail&operType=planDetail&billRecordFlag=${rs.record_flag}&ModuleID=${ModuleID}', '', 1200, 800)">
                            ...
                            <td  id="out_in_diff" class="table_list_tr_col_data_block">
                                <c:if test = "${rs.out_in_diff != 0}">
                                    <a href='#'
                                       onClick="openwindow('ticketStorageInCheckManage?queryCondition_in=${rs.bill_no}&queryCondition_out=${rs.related_bill_no}&command=queryDiff&operType=diff&billRecordFlag=${rs.record_flag}&ModuleID=${ModuleID}', '', 1200, 800)">
                                        <!--  <a href='#' onClick="window.showModelessDialog('ticketStorageOutProduceManage.do?queryCondition={billNo}&amp;command=queryDetail&amp;operType=planDetail&amp;ModuleID={/Service/Result/ModuleID}','0','dialogWidth:700px;dialogHeight:500px;center:yes;resizable:no;status:no;scroll:no')"> --> 
                                        ${rs.out_in_diff}
                                    </a>                                
                                </c:if>         
        ...
        <FORM method="post" name="detailOp" id="detailOp" action="ticketStorageInCheckManage" >
            ...
            <input type="hidden" name="expFileName" id="d_expFileName" value="核查入库.xlsx" />
            <input type="hidden" name="divId" id="d_divId" value="clearStartHead">
            <input type="hidden" name="methodUrl" id="d_methodUrl" value="/TicketStorageInCancelExportAll"/>
            ...
            <div id="detail"  class="divForTableDataDetail" >
                <table  class="table_edit_detail">
                    <tr class="table_edit_tr">
                        ...
                        <td class="table_edit_tr_td_input">
                            <select id="d_related_bill_no" name="d_related_bill_no" dataType="Require" msg="出库单号不能为空!" >
                                <c:import url="/jsp/common/common_template.jsp?template_name=option_list_pub_cancelOutBill" />
                            </select>
                        </td>
                    ...
                    <tr class="table_edit_tr">
                        ...
                        <td class="table_edit_tr_td_input">
                            <input type="text" name="d_accounter" id="d_accounter" size="10" require="true" maxlength="45"dataType="NotEmpty|LimitContainChinese" min="1" max="45" msg="记帐人不能为空,且长度不超过45个字节,或15个汉字"/>
                        </td>
