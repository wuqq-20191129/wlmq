 ...
 * @author zhangjh
 */
public class HanderTHD extends HanderTk {
	private static final Logger logger = Logger.getLogger(HanderTHD.class.getName());

	@Override
	public void handleMessage(HandleMessageBase msg) {
		tradType = msg.getTradType();
		String fileName = msg.getFileName();
		logger.info("车站上交数据文件解析处理开始，文件名：" + fileName);
		doParse(msg, fileName);
		logger.info("车站上交数据文件解析处理结束，文件名：" + fileName);
	}
