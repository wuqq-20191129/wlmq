...
BEGIN
  ...
  --运营时间
  SELECT LPAD(TRIM(squad_time),4,'0') INTO TMP_SHH FROM acc_st.st_cfg_sys WHERE delete_flag = '0';
  --参数时间（时分）
  ...
  IF TMP_STATION_ID != '-1' THEN
      ...
      BEGIN
          ...
          IF V_TYPE = 'MAIN' THEN
            OPEN INFO_CUR FOR
            SELECT nvl(count(t.traffic),0) traffic,t.flag,lpad(t.card_main_type,2,'0') card_type,s.card_main_name
          ...
          ELSE
            ...
            OPEN INFO_CUR FOR
            SELECT nvl(count(t.traffic),0) traffic,t.flag,lpad(t.card_main_type,2,'0')||lpad(t.card_sub_type,2,'0') card_type,s.card_sub_name
