...
import com.goldsign.commu.app.dao.Parameter0604Dao;
...
public class Parameter0604 extends ParameterBase {
	...
	private static Logger logger = Logger.getLogger(Parameter0604.class
			.getName());

	@Override
	public boolean formParaFile() {
		boolean result;
		logger.info("参数0604 started! ");
		try {
			// get parameter data
			Vector<String[]> recV = Parameter0604Dao.getRecordFromTable(paraGenDtl,
					dbHelper, FORMAT.length);
			result = dealData(recV, PARMTYPE, FORMAT);
		} catch (Exception e) {
			logger.error("参数0604 error! " + e);
			result = false;
		}
		logger.info("参数0604 ended! ");
		return result;
	}
}
