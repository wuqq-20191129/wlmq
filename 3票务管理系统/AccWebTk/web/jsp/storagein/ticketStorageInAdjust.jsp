    ...
    <head>
        ...
        <script language="javascript" type="text/javascript">
                function openWindowDialogByDisabled(formName) {
                    var handMan = document.forms[formName].getElementsByTagName("input")['d_hand_man'];
                    if (handMan.disabled == true) {
                        return; 
                    } else {
                        openWindowDialog('detailOp','d_related_bill_no','ticketStorageInAdjustCheck?queryCondition=${rs.related_bill_no}&command=query&operType=checkInSelect&ModuleID=${ModuleID}',900,600);
                    }
                }
        </script>
...
<!--    onload="initDocument('formQuery','detail');initDocument('formOp','detail');
            ...
            setPrimaryKeys('formOp','d_related_bill_no#d_reason_id');">-->
    <body onload="initDocument('queryOp', 'detail');
        ...
        <form method="post" name="queryOp" id="queryOp" action="ticketStorageInAdjustManage">
            <!-- 页面用到的变量 通用模板 -->
            ...
            <table  class="table_edit">
                <tr class="table_edit_tr">
                    <input type="hidden" name="inType" value="TR"  />
...
<!--                                            controlByFlag('detailOp',['audit']);
                                            controlByFlag('detailOp',['del'])"-->
                            </td>
                            
                            <td  id="billNo" class="table_list_tr_col_data_block" style="width: 90px">
                                <a href='#'
                                    onClick="openwindow('ticketStorageInAdjustDetail?billRecordFlag=${rs.record_flag}&queryCondition=${rs.bill_no}&command=query&ModuleID=${ModuleID}','',1000,600)">
                                    ${rs.bill_no}
                                </a>
                            </td>
                            ...
                            <td  id="related_bill_no" class="table_list_tr_col_data_block" style="width: 90px">
                                <a href='#'
                                    onClick="openwindow('ticketStorageInAdjustCheck?queryCondition=${rs.related_bill_no}&command=query&operType=checkIn&ModuleID=${ModuleID}','',1000,600)">
                                    ${rs.related_bill_no}
                                </a>
                            </td>
        ...
        <FORM method="post" name="detailOp" id="detailOp" action="ticketStorageInAdjustManage" >
            ...
            <input type="hidden" name="expAllFields" id="d_expAllFields"
                value="getBill_no,getRelated_bill_no,getBill_date,getForm_maker,getHand_man,getVerify_date,getVerify_person,getRecord_flag_name,getRemark" />
            <!--导出文件名字 生成的Excel sheet的名字也是这个-->
            <input type="hidden" name="expFileName" id="d_expFileName" value="调账入库.xlsx" />
            <!--需要导出页面的表数据  一个页面多个导出全部根据这里设置详情参看运营系统退款模块-->
            <input type="hidden" name="divId" id="d_divId" value="clearStartHead">
            <!--对应你controller写的方法  一个页面多个导出全部根据这里设置详情参看运营系统退款模块-->
            <input type="hidden" name="methodUrl" id="d_methodUrl" value="/ticketStorageInAdjustManageExportAll"/>
            ...
            <div id="detail"  class="divForTableDataDetail" >
                <table  class="table_edit_detail">
                    <tr class="table_edit_tr">
                        ...
                        <td class="table_edit_tr_td_input">
                            <select id="d_reason_id" name="d_reason_id" dataType="Require" msg="调账原因不能为空!" value="16">
                                <c:import url="/jsp/common/common_template.jsp?template_name=option_list_pub_inreason_inAdjust" />
                            </select>	
                        </td>
                        <td class="table_edit_tr_td_label">盘点人:</td>
                        <td class="table_edit_tr_td_input">
                            <input type="text" id="d_hand_man" name="d_hand_man" size="15"   dataType="Required|LimitContainChinese" maxlength="45" min="1" max="45" msg="盘点人不能为空且最大长度为45个字节，一个中文是3个字节(最大15个中文)"/>
                        </td>
