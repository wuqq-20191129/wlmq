 ...
 * @author hejj
 */
public class HandlerREG extends HandlerBase {

    private static Logger logger = Logger.getLogger(HandlerREG.class.getName());
    ...
    private void parseFileName(String fileName, String balanceWaterNo) throws FileNameException {
        ...
        util.parseLen(fileName, FileNameParseUtil.FILE_LEN_REG);
        util.parseFmtForOneLineStation(fileName, FileNameParseUtil.FILE_START_REG);
        util.parseDateIndexTwo(fileName, balanceWaterNo);
        util.parseRepeat(fileName);

    }

    private void addDetail(FileData fd) {
        ...
        for (Object tradType : hm.keySet()) {
            ...
            this.addDetail(tradTypeStr, v, hm);

        }

    }

    private void addDetail(String tradType, Vector records, HashMap hm) {
        String tradTypeSub = tradType + HandlerBase.TRAD_SUFIX[0];
        FileRecord81 record;
        Vector v;
        for (int i = 0; i < records.size(); i++) {
            record = (FileRecord81) records.get(i);
            v = record.getDealDetail();
            this.putMap(tradTypeSub, v, hm);

        }
    }

    private void addInfo(FileData fd) throws Exception {
        HashMap hm = fd.getContent();
        Vector v;
        ...
        for (Object tradType : hm.keySet()) {
            ...
            this.addInfoForSeq(v, result.getRetValues());

        }

    }

    private void addInfoForSeq(Vector records, Vector seqs) {
        String seq;
        FileRecord81 record;
        for (int i = 0; i < records.size(); i++) {
            record = (FileRecord81) records.get(i);
            seq = (String) seqs.get(i);
            record.setWaterNo(seq);
            this.addRecordDetailForSeq(record.getDealDetail(), seq);

        }

    }

    private void addRecordDetailForSeq(Vector details, String seq) {

        FileRecord81Detail detail;
        for (int i = 0; i < details.size(); i++) {
            detail = (FileRecord81Detail) details.get(i);
            detail.setWaterNo(seq);
        }
    }
