 ...
 * @author zhangjh
 */
public class Parameter0602Dao {

	private static Logger logger = Logger.getLogger(Parameter0602Dao.class
			.getName());
	private static String sqlStr = "select begin_logical_id,end_logical_id,action_type "
			+ " from "+FrameDBConstant.COM_ST_P+"op_prm_black_list_mtr_sec "
	...
	public static Vector<String[]> getRecordFromTable(ParaGenDtl paraGenDtl,
		...
		try {
			...
			while (result) {
				...
				if(Parameter06Util.patternMacher(fields[0]) && Parameter06Util.patternMacher(fields[1])){
                                    recV.add(fields);
                                } else {
			...
			logger.error("Access table "+FrameDBConstant.TABLE_PREFIX+"op_prm_black_list_mtr_sec error! " + e);
			return null;
		}
                // 黑名单段数量限定
		recV = Parameter06Util.getRecordNotOverLimit(recV, Parameter06Util.blacklistSecMax);
                Parameter06Util.inErrBlackDownSEC(errV,"1");
