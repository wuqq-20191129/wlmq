...
<c:choose>
    ...
    <c:when test="${templateName == 'option_list_pub_samtype'}">
        <!-- SAM卡类型 20170824  ldz-->
        <option value="">=请选择=</option>
    ...
    <c:when test="${templateName == 'option_list_pub_produce_type'}">
        <!-- 产品类型 20170824  chenzx-->
        <option value="">=请选择=</option>
        <c:forEach items="${produceTypes}" var="produceType"> 
            <c:choose>
                <c:when test="${produceType.code_text == ''}">
                    <option value="${produceType.code}">${produceType.code}</option>
                </c:when>
                <c:otherwise>
                    <option value="${produceType.code}">${produceType.code_text}</option>
                </c:otherwise>
            </c:choose>
        </c:forEach>
    </c:when>

    <c:when test="${templateName == 'option_list_pub_stock_state'}">
        <!-- 库存状态 20170824  chenzx-->
        <option value="">=请选择=</option>
        <c:forEach items="${stockStates}" var="stockState"> 
            <c:choose>
                <c:when test="${stockState.code_text == ''}">
                    <option value="${stockState.code}">${stockState.code}</option>
                </c:when>
                <c:otherwise>
                    <option value="${stockState.code}">${stockState.code_text}</option>
                </c:otherwise>
            </c:choose>
        </c:forEach>
    </c:when>

    <c:when test="${templateName == 'option_list_pub_is_instock'}">
        <!-- 是否在库 20170824  chenzx-->
        <option value="">=请选择=</option>
        <c:forEach items="${isInstocks}" var="isInstock"> 
            <c:choose>
                <c:when test="${isInstock.code_text == ''}">
                    <option value="${isInstock.code}">${isInstock.code}</option>
                </c:when>
                <c:otherwise>
                    <option value="${isInstock.code}">${isInstock.code_text}</option>
                </c:otherwise>
            </c:choose>
        </c:forEach>
    </c:when>

    <c:when test="${templateName == 'option_list_pub_is_bad'}">
        <!-- 是否损坏 20170824  chenzx-->
        <option value="">=请选择=</option>
        <c:forEach items="${isBads}" var="isBad"> 
            <c:choose>
                <c:when test="${isBad.code_text == ''}">
                    <option value="${isBad.code}">${isBad.code}</option>
                </c:when>
                <c:otherwise>
                    <option value="${isBad.code}">${isBad.code_text}</option>
                </c:otherwise>
            </c:choose>
        </c:forEach>
    </c:when>
    ...
    <c:when test="${templateName == 'option_list_pub_line_es'}">
        <!-- 线路(ES) 20170828  luck-->
        <option value="">=请选择=</option>
        <c:forEach items="${linees}" var="linees"> 
            <c:choose>
                <c:when test="${linees.code_text == ''}">
                    <option value="${linees.code}">${linees.code}</option>
                </c:when>
                <c:otherwise>
                    <option value="${linees.code}">${linees.code_text}</option>
                </c:otherwise>
            </c:choose>
        </c:forEach>
    </c:when>

    <c:when test="${templateName == 'option_list_pub_distribute_place'}">
        <!-- 分发目的地 20170831  zhouy-->
        <option value="">=请选择=</option>
        <c:forEach items="${distributePlace}" var="distributePlace"> 
            <c:choose>
                <c:when test="${distributePlace.code_text == ''}">
                    <option value="${distributePlace.code}">${distributePlace.code}</option>
                </c:when>
                <c:otherwise>
                    <option value="${distributePlace.code}">${distributePlace.code_text}</option>
                </c:otherwise>
            </c:choose>
        </c:forEach>
    </c:when>

    <c:when test="${templateName == 'option_list_pub_issue_OrderNO'}">
        <!-- 卡发行单号 20170831  zhouy-->
        <option value="" id="produceOrderSelf">=请选择=</option>
        <c:forEach items="${issueOrderNo}" var="issueOrderNo"> 
            <c:choose>
                <c:when test="${issueOrderNo.code_text == ''}">
                    <option value="${issueOrderNo.code}">${issueOrderNo.code}</option>
                </c:when>
                <c:otherwise>
                    <option value="${issueOrderNo.code}">${issueOrderNo.code_text}</option>
                </c:otherwise>
            </c:choose>
        </c:forEach>
    </c:when>
    ...
    <c:when test="${templateName == 'option_list_pub_line_es_Type'}">
        <!-- 线路es类型 20170831 ysw-->
        <option value="">=请选择=</option>
        <c:forEach items="${lineEsTypes}" var="lineEsType"> 
            <c:choose>
                <c:when test="${lineEsType.code_text == ''}">
                    <option value="${lineEsType.code}-${lineEsType.code_text}">${lineEsType.code}</option>
                </c:when>
                <c:otherwise>
                    <option value="${lineEsType.code}-${lineEsType.code_text}">${lineEsType.code_text}</option>
                </c:otherwise>
            </c:choose>
        </c:forEach>
    </c:when>

    <c:when test="${templateName == 'option_list_pub_samtype_desc'}">
        <!-- 卡类型定义名称 20180328  ldz-->
        <option value="">=请选择=</option>
        <c:forEach items="${samTypeDescs}" var="samTypeDescs"> 
            <c:choose>
                <c:when test="${samTypeDescs.code_text == ''}">
                    <option value="${samTypeDescs.code}">${samTypeDescs.code}</option>
                </c:when>
                <c:otherwise>
                    <option value="${samTypeDescs.code}">${samTypeDescs.code_text}</option>
                </c:otherwise>
            </c:choose>
        </c:forEach>
    </c:when>
