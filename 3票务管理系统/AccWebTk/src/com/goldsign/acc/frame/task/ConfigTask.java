    ...
    */
    ...
    public void authenRefresh(){
      ...
      //add by zhongziqi 20180202
        ...
        //add by moqf 20190912 设置票务报表服务器机器名，与清算报表服务器相同
        key.setTypeSub("6");
        configs = configMapper.selectConfigs(key);
        for(Config config:configs){
             String configName = config.getConfigName();
            if(configName.equals(ReportConstant.REPORT_SERVER_NAME_KEY)) {
            	ReportConstant.REPORT_SERVER_NAME = config.getConfigValue();
            	continue;
            }
        }
