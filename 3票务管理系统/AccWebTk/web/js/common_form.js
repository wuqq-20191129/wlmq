...
//        return false;
    ...
    headEventObject.style.background = "#74BB6B";//"#97CBFF";
...
function getCurrentLine(formName) {
    ...
    var rowSelected = frm.children["rowSelected"]; //使用childrenrow，Selected 标签只能放在DIV里面才找到
    if (rowSelected != null) {
                                        ...
                                        switch (tmpObj.type) {
                                            ...
                                            case "text":
                                            {
                                                ...
                                                if (child.children[0] != null && child.children[0].tagName == 'A') {
                                                    tmpObj.value = child.children[0].innerText;
                                                }
...
function btnClickForMulDataAudit(formName, listName, detailName) {
...
//			alert("btExport");
        ...
        case "btAudit":
        {
            if (!checkAudit(listName)) {
                alert("一次仅能审核一条记录，请重新选择！");
                return;
            }
            var rst = confirm("确定对所选记录审核？");
            if (!rst)
...
function exportBillDetail(formName, template, paramNames, paramValueNames, iWidth, iHeight) {
    ...
    var url = 'ticketStorageExportBillManage'; //转向网页的地址;
    var name; //网页名称，可为空;
    var iWidth; //弹出窗口的宽度;
    var iHeight; //弹出窗口的高度;
...
//过滤重复的数据，如单号 20180104 mqf
//listName 列表的DIV ID； 存储选择项ID的对象名称应为 allSelectedIDs；行ID必须存储记录ID
function getAllSelectedIDsForNoRepeat(listName, formName) {
//	alert('call');
    var listDiv = document.getElementById(listName);
    var tbl = listDiv.children[0];
    var selectedIDs = document.forms[formName].getElementsByTagName("input")['allSelectedIDs'];
...
//  alert(selectedCK.checked);
            if (selectedCK.checked) {
                if (selectedIDs.value.indexOf(tblRow.id)==-1) { //过滤重复的数据
                    selectedIDs.value += tblRow.id + ";";
                    //				alert(selectedIDs.value);
                }
            }
...
//票柜定义模块   票卡主类型选择后，子类型默认为空
function setSelectValuesNoSelDefault(formName, selectedName, selectName, varName) {
//	     alert("formName="+formName);
//	      alert("varName="+varName);
//	      alert("varValues="+document.forms[formName].children[varName].value);
...
//	      alert("varValues="+document.forms[formName].children[varName].value);
    ...
    for (i = 0; i < values.length; i++) {
        ...
        if (isLegalValue(objs[1], subValues)) {
            var opt = new Option(objs[2].trim(), objs[1].trim());
...
//    var rowID = rowOb.getAttribute(rowAttributeName);
    var rowID = rowOb.getAttribute(rowAttributeName).trim();
        ...
        for (j = 0; j < colCount; j++)
        {
            if (i == 0) {
                ...
                if (table.rows[0].cells[j].children[0] == null) {
                    ...
                    if(i==0){
                        oSheet.Cells(l + 1, k + 1).Font.Bold = true; //设置为粗体*
                         oSheet.Cells(l + 1, k + 1).ColumnWidth = 2*cellText.length; //表头不换行
                    }
                    oSheet.Cells(l + 1, k + 1).Borders.Weight = 2; //设置单元格边框*()
...
function clickOneResultRow(formName, object, detailFrom, name) {
    ...
    // 改变背景色
    rowBackGround=headEventObject.style.backgroundColor;
    headEventObject.style.background = "#74BB6B";
