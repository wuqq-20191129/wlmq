...
//                style.setAlignment(HorizontalAlignment.CENTER);
    ...
    /**
     * 针对入库单，转化指定成员信息entity为Map
     *
     * @param fields 方法集
     * @param results 结果集
     */
    public static List<Map<String, Object>> entityToMapForInBill(String expAllFields, List results, String billNoPrefix) throws Exception {
        if (expAllFields == null) {
            throw new Exception("字段数据为空");
        }
        String[] fields = expAllFields.split(",");
        List<Map<String, Object>> list = new ArrayList<>();
        //"0"： 初始值，"1"：是当前模块的入库类型， "2":非当前模块的入库类型
        String isCurInBillType = "0";
        String fieldNameBillNo = "getBill_no";
        if (results.size() > 0 || fields.length == 0) {
            for (Object vo : (List<Object>) results) {
                Class<?> entity = vo.getClass();
                Map<String, Object> map = new HashMap<>();
                for (int i = 0; i < fields.length; i++) {
                    if (fields[i] != null && fields[i].length() != 0) {
                        ...
                        map.put(fields[i], method.invoke(vo));
                        //获取单号，判断是否当前模块的入库类型
                        if (isCurInBillType.equals("0") && fields[i].equals(fieldNameBillNo)) {
                            String billNo = (String)method.invoke(vo);
                            if (billNo != null && billNo.startsWith(billNoPrefix)) {
                                isCurInBillType = "1";
                            } else {
                                isCurInBillType = "2";
                            }
        ...
        //如果缓存数据不是当前模块的入库类型，返回空记录。
        if (isCurInBillType.equals("0") || isCurInBillType.equals("2")) {
            list = new ArrayList<>();
        } 
        return list;
    }
