...
import com.goldsign.acc.app.opma.mapper.SignCardLossApplyHandleHisMapper;
import com.goldsign.acc.app.opma.mapper.SignCardLossApplyHandleMapper;
 ...
 */
@Controller
public class SignCardLossApplyHandleController extends CodePubFlagController {
    @Autowired
    private SignCardLossApplyHandleMapper mapper;
    @Autowired
    private SignCardLossApplyHandleHisMapper hisMapper;

    @Autowired
    private SignCardLossApplyOpLogMapper logMapper;

    @RequestMapping("/SignCardLossApplyHandle")
    public ModelAndView service(HttpServletRequest request, HttpServletResponse response) {
        ModelAndView mv = new ModelAndView("/jsp/opma/sign_card_loss_apply_handle.jsp");
        String command = request.getParameter("command");
        //判断查询后是否有点击一条记录
        String selectRowFlag = request.getParameter("selectRowFlag");
        OperationResult opResult = new OperationResult();
        //第二列表的数据
        List<SignCardLossApplyHandleHis> hisResult = new Vector();
        //用于点击后显示处理框数据
        SignCardLossApplyHandle selectVo = new SignCardLossApplyHandle();
        //用于保存修改数据
//        SignCardLossApplyHandle detailVo = new SignCardLossApplyHandle();
        try {
            if (command != null) {
                command = command.trim();
                //普通查询
                if (command.equals(CommandConstant.COMMAND_QUERY) && NonReturnApplyHandleConstant.FLAG_FALSE.equals(selectRowFlag)) {
                    opResult = this.query(request, this.mapper, this.operationLogMapper);
                }//点击一条记录
                if (command.equals(CommandConstant.COMMAND_QUERY) && NonReturnApplyHandleConstant.FLAG_TRUE.equals(selectRowFlag)) {
                ...
                if (command.equals(CommandConstant.COMMAND_CONFIRM_REFUND)) {
                    ...
                    selectVo = this.getResultByBusinessReceiptIdAfterConfirm(opResult, request, this.operationLogMapper);
                    hisResult = this.getHisResult(request,selectVo, hisMapper, this.operationLogMapper);
                    if (i == 0) {
    ...
    private OperationResult query(HttpServletRequest request, SignCardLossApplyHandleMapper mapper, OperationLogMapper operationLogMapper) throws Exception {
        OperationResult or = new OperationResult();
        LogUtil logUtil = new LogUtil();
        SignCardLossApplyHandle queryCondition;
        List<SignCardLossApplyHandle> resultSet;
        try {
            queryCondition = this.getQueryCondition(request);
            resultSet = mapper.getSignCardLossApplyQuery(queryCondition);
    ...
    private SignCardLossApplyHandle getQueryCondition(HttpServletRequest request) {
        SignCardLossApplyHandle vo = new SignCardLossApplyHandle();
        ...
        String qQueryCondition = request.getParameter("q_queryCondition");
        ...
        if (!"".equals(qQueryCondition) && !"".equals(businessReceiptId)) {
            if (qQueryCondition.equals(NonReturnApplyHandleConstant.QUERY_BY_BUSINESS_RECEIPT_ID)) {
                vo.setBusinessReceiptId(businessReceiptId);
            }
            if (qQueryCondition.equals(NonReturnApplyHandleConstant.QUERY_BY_CREDENTIALS)) {
                vo.setIdentityId(businessReceiptId);
            }
        }
    ...
    private void setResultText(ModelAndView mv) {
        List<SignCardLossApplyHandle> resultSet = (List<SignCardLossApplyHandle>) mv.getModelMap().get(WebConstant.ATT_ResultSet);
        ...
        for (SignCardLossApplyHandle vo : resultSet) {
            ...
            if(idTypes!=null&&!idTypes.isEmpty()){
                 vo.setIdentityTypeText(this.getTextByCode(vo.getIdentityType(), idTypes));
             }

        }

        List<SignCardLossApplyHandleHis> hisResultSet = (List<SignCardLossApplyHandleHis>) mv.getModelMap().get("hisResult");
        if (hisResultSet == null || hisResultSet.isEmpty()) {
            return;
        }
        for (SignCardLossApplyHandleHis vo : hisResultSet) {
        ...
        SignCardLossApplyHandle detaileVo = (SignCardLossApplyHandle) mv.getModelMap().get("selectVo");
        if(detaileVo!=null){
            if (stations != null && !stations.isEmpty()) {
                detaileVo.setStationText(DBUtil.getTextByCode(detaileVo.getStationId(), detaileVo.getLineId(), stations));
            }
            if (handleFlags != null && !handleFlags.isEmpty()) {
                detaileVo.setHdlFlagText(this.getTextByCode(detaileVo.getHdlFlag(), handleFlags));
            }
            if (penatlyReasons != null && !penatlyReasons.isEmpty()) {
                detaileVo.setPenaltyReasonText(this.getTextByCodeAdaptNull(detaileVo.getPenaltyReason(), penatlyReasons));
            }
            if (auditFlags != null && !auditFlags.isEmpty()) {
    ...
    private void setBrokenFlag(ModelAndView mv, SignCardLossApplyHandleMapper mapper) {
        List<CodePubFlag> brokenFlags = this.getBrokenFlag();
        mv.addObject("brokenFlags", brokenFlags);
    }

    private void setHandleFlag(ModelAndView mv, SignCardLossApplyHandleMapper mapper) {
    ...
    private void setHandleType(ModelAndView mv, SignCardLossApplyHandleMapper mapper) {
        List<CodePubFlag> handleTypes = this.getHandleType();
        mv.addObject("handleTypes", handleTypes);
    }

    private void setQueryCondition(ModelAndView mv, SignCardLossApplyHandleMapper mapper) {
        List<CodePubFlag> queryConditons = this.getQueryConditionForSignCard();
        mv.addObject("queryConditons", queryConditons);
    }

    private void setAuditFlag(ModelAndView mv, SignCardLossApplyHandleMapper mapper) {
        List<CodePubFlag> auditFlags = this.getAuditFlag();
        mv.addObject("auditFlags", auditFlags);
    }

    private SignCardLossApplyHandle getResultByBusinessReceiptId(OperationResult opResult, HttpServletRequest request, OperationLogMapper operationLogMapper) throws Exception {
        SignCardLossApplyHandle vo = new SignCardLossApplyHandle();
        LogUtil logUtil = new LogUtil();
        try {
            String selectBusinessReceiptId = request.getParameter("selectReceiptId").trim();

            List<SignCardLossApplyHandle> resultSet = opResult.getReturnResultSet();
            if ((resultSet != null) && !resultSet.isEmpty()) {
                if (!"".equals(selectBusinessReceiptId)) {
                    for (SignCardLossApplyHandle po : resultSet) {
    ...
    private List<SignCardLossApplyHandleHis> getHisResult(HttpServletRequest request,SignCardLossApplyHandle selectVo, SignCardLossApplyHandleHisMapper hisMapper, OperationLogMapper operationLogMapper) throws Exception {
        List<SignCardLossApplyHandleHis> result = new Vector();
        LogUtil logUtil = new LogUtil();
        if (selectVo.getBusinessReceiptId() != null && selectVo.getBusinessReceiptId().trim().length() == NonReturnApplyHandleConstant.BUSINESS_RECEIPT_ID_LENGTH) {
            try {
    ...
    private void setDetailVo(ModelAndView mv, SignCardLossApplyHandle selectVo) {
        mv.addObject("selectVo", selectVo);
    }

    private void isApplied(SignCardLossApplyHandle vo) {
    ...
    private void getPenatly(SignCardLossApplyHandle vo) {
        // 未处理
        String penatly = "";
    ...
    private void isBlackCard(SignCardLossApplyHandle vo) {
        ...
        if (checkInBlackListMtrSec.equals(NonReturnApplyHandleConstant.FLAG_NO) && checkInBlackListMtr.equals(NonReturnApplyHandleConstant.FLAG_NO)) {
    ...
    private void getDeposit(SignCardLossApplyHandle vo) {
    ...
    private void getBalance(SignCardLossApplyHandle vo) {
        SignCardLossApplyHandle balanceVo = mapper.getBalance(vo);
        if (balanceVo == null || balanceVo.getCardLogicalId() == null) {
            SignCardLossApplyHandleHis hisVo = mapper.getHisBalance(vo);
    ...
    private void setDefaultBrokenFlag(SignCardLossApplyHandle vo) {
    ...
    private int confirmRefund(OperationResult opResult, HttpServletRequest request, SignCardLossApplyHandleMapper mapper, OperationLogMapper operationLogMapper) throws Exception {
        LogUtil logUtil = new LogUtil();
        int result = 0;
        try {
...
//                17
    ...
    private void setDealType(ModelAndView mv, SignCardLossApplyHandleMapper mapper) {
        List<CodePubFlag> dealTypes = this.getDealType();
        mv.addObject("dealTypes", dealTypes);
    }

    private void setHisResult(ModelAndView mv, List<SignCardLossApplyHandleHis> hisResult, SignCardLossApplyHandleMapper mapper) {
        if (hisResult != null && !hisResult.isEmpty()) {
            ...
            if (limitRow >= hisResult.size()) {
                ...
                List<SignCardLossApplyHandleHis> displyHisResult = new Vector();
                //先加固定的行数
                ...
                for (int i = limitRow; i < hisResult.size(); i++) {
                    SignCardLossApplyHandleHis po = hisResult.get(i);
    ...
    private SignCardLossApplyHandle getDetailMsg(HttpServletRequest request) {
        SignCardLossApplyHandle vo = new SignCardLossApplyHandle();
    ...
    private OperationResult queryForOp(HttpServletRequest request, SignCardLossApplyHandleMapper mapper, OperationLogMapper operationLogMapper, OperationResult or) throws Exception {
        LogUtil logUtil = new LogUtil();
        SignCardLossApplyHandle queryCondition;
        List<SignCardLossApplyHandle> resultSet;
        try {
    ...
    private SignCardLossApplyHandle getQueryConditionForOp(HttpServletRequest request) {
        SignCardLossApplyHandle vo = new SignCardLossApplyHandle();
        HashMap vQueryControlDefaultValues = null;
        String queryControlDefaultValues = request.getParameter("queryControlDefaultValues");
        vQueryControlDefaultValues = FormUtil.getQueryControlDefaultValues(queryControlDefaultValues);
        if (!vQueryControlDefaultValues.isEmpty()) {
            String businessReceiptId = FormUtil.getQueryControlDefaultValue(vQueryControlDefaultValues, "q_businessReceiptId");
            ...
            String queryCondition = FormUtil.getQueryControlDefaultValue(vQueryControlDefaultValues, "q_queryCondition");
    ...
    private SignCardLossApplyHandle getResultByBusinessReceiptIdAfterConfirm(OperationResult opResult, HttpServletRequest request, OperationLogMapper operationLogMapper) throws Exception {
        SignCardLossApplyHandle vo = new SignCardLossApplyHandle();
        LogUtil logUtil = new LogUtil();
        try {
            String businessReceiptId = request.getParameter("detailReceiptId").trim();
            vo = mapper.getSignCardLossApplyQueryByBusinessReciptId(businessReceiptId);
            this.isBlackCard(vo);
            this.isApplied(vo);
            this.setDefaultBrokenFlag(vo);
        } catch (Exception e) {
    ...
    private void writeOperatorLog(SignCardLossApplyHandle selectVo, HttpServletRequest request, String type) throws Exception {
    ...
    private int refusedRefund(OperationResult opResult, HttpServletRequest request, SignCardLossApplyHandleMapper mapper, OperationLogMapper operationLogMapper) throws Exception {
        LogUtil logUtil = new LogUtil();
        int result = 0;
        try {
    ...
    private SignCardLossApplyHandle getDetailMsgForRefused(HttpServletRequest request) {
        SignCardLossApplyHandle vo = new SignCardLossApplyHandle();
        String detailReceiptId = (String) request.getParameter("detailReceiptId");
        String remark = (String) request.getParameter("d_remark");
        ...
        String hdlFLag = "1";
        if (NonReturnApplyHandleConstant.FLAG_NORMAL_CARD.equals(blackCardFlag)) {
            hdlFLag = "4";
        }else {//增加标识
            remark = remark + " 拒绝退款";
        }
    ...
    private int confirmAudit(OperationResult opResult, HttpServletRequest request, SignCardLossApplyHandleMapper mapper, OperationLogMapper operationLogMapper) throws Exception {
        LogUtil logUtil = new LogUtil();
        int result = 0;
        try {
    ...
    private SignCardLossApplyHandle getDetailMsgForAudit(HttpServletRequest request) {
        SignCardLossApplyHandle vo = new SignCardLossApplyHandle();
        String detailReceiptId = (String) request.getParameter("detailReceiptId");
        vo.setBusinessReceiptId(detailReceiptId);
        return vo;
    ...
    private int confirmModify(OperationResult opResult, HttpServletRequest request, SignCardLossApplyHandleMapper mapper, OperationLogMapper operationLogMapper) throws Exception {
        SignCardLossApplyHandle vo = new SignCardLossApplyHandle();
        LogUtil logUtil = new LogUtil();
        int result = 0;
        try {
            ...
            e.printStackTrace();
            logUtil.operationExceptionHandle(request, CommandConstant.COMMAND_CONFIRM_MODIFY, e, operationLogMapper);
        }
//        if (result == 1) {
//            logUtil.logOperation(CommandConstant.COMMAND_CONFIRM_MODIFY, request, LogConstant.confirmModifySuccessMsg(result),
//                    operationLogMapper);
//        }
        return result;
    }

    private SignCardLossApplyHandle getDetailMsgForModify(HttpServletRequest request) {
        SignCardLossApplyHandle vo = new SignCardLossApplyHandle();
        String detailReceiptId = (String) request.getParameter("detailReceiptId");
        String remark = (String) request.getParameter("d_remark");
    ...
    private void setIdType(ModelAndView mv, SignCardLossApplyHandleMapper mapper) {
        List<CodePubFlag> idTypes =mapper.getIdTypes();
         mv.addObject("idTypes",idTypes);
    }


	@RequestMapping("/SignCardLossApplyHandleExportAll")
	public void exportExcel(HttpServletRequest request, HttpServletResponse response) throws Exception {
		SignCardLossApplyHandle queryCondition = null;
		if (request.getSession().getAttribute(NonReturnApplyHandleConstant.EXPORT_EXCEL_VO_KEY_FIRST) != null) {
			...
			if (obj instanceof SignCardLossApplyHandle) {
				queryCondition = (SignCardLossApplyHandle) request.getSession().getAttribute(NonReturnApplyHandleConstant.EXPORT_EXCEL_VO_KEY_FIRST);
			}
		}
		...
		for (Map vo : queryResults) {
			...
			if(idTypes!=null&&!idTypes.isEmpty()){
                vo.put("IDENTITY_TYPE",this.getTextByCode(vo.get("IDENTITY_TYPE").toString(), idTypes));
            }
			// 转换申请时间
	...
	@RequestMapping("/SignCardLossApplyHandleHisExportAll")
	public void exportHisExcel(HttpServletRequest request, HttpServletResponse response) throws Exception {
		SignCardLossApplyHandle queryCondition = new SignCardLossApplyHandle();
		...
		if (request.getSession().getAttribute(NonReturnApplyHandleConstant.EXPORT_EXCEL_VO_KEY_SECOND) != null) {
			...
			if (obj instanceof SignCardLossApplyHandle) {
				queryCondition = (SignCardLossApplyHandle) request.getSession().getAttribute(NonReturnApplyHandleConstant.EXPORT_EXCEL_VO_KEY_SECOND);
			}
		}else {
		    //一开始不能导出数据
