...
<c:choose>
    ...
    <c:when test="${templateName == 'option_list_pub_recordFlags'}">
        <!-- 版本状态 add by luck 20170906 -->
        <option value="">==全部==</option>
        <c:forEach items="${recordFlags}" var="recordFlag"> 
            <c:choose>
                <c:when test="${recordFlag.code_text == ''}">
                    <c:choose>
                        <c:when test="${recordFlag.code == '0'}">
                            <option value="${recordFlag.code}">${recordFlag.code}</option>
                        </c:when>
                        <c:otherwise>
                            <option value="${recordFlag.code}">${recordFlag.code}</option>
                        </c:otherwise>
                ...
                <c:otherwise>
                    <c:choose>
                        <c:when test="${recordFlag.code == '0'}">
                            <option value="${recordFlag.code}" selected="selected">${recordFlag.code_text}</option>
                        </c:when>
                        <c:otherwise>
                            <option value="${recordFlag.code}">${recordFlag.code_text}</option>
                        </c:otherwise>
                    </c:choose>
                </c:otherwise>
            </c:choose>
    ...
    <c:when test="${templateName == 'op_button_list_all_init'}">
        ...
        <c:set var="generate" scope="request" value="0"/>
        <c:set var="generateOD" scope="request" value="0"/>
        <c:set var="fileDown" scope="request" value="0"/>
        ...
        <table width="95%" align="center">
            <tr>
                <td>
                    ...
                    <c:if test="${generate == 1}"><input type="button" id="btGenerate" name="generate"  value="生成权重数据" class="buttonStyle" onclick="${clickMethod}" /></c:if>
                    <c:if test="${generateOD == 1}"><input type="button" id="btGenerateOD" name="generateOD"  value="生成OD数据" class="buttonStyle" onclick="${clickMethod}" /></c:if>
                    <c:if test="${fileDown == 1}"><input type="button" id="btFileDown" name="fileDown"  value="参数下载" class="buttonStyle" onclick="${saveFile}" /></c:if>
