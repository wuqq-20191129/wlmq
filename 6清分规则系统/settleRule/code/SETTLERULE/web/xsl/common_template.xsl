...
<xsl:template name="common_status_table">
    ...
    <table width="95%" align="center"  class="statusStyle" >
            ...
            <tr bgcolor="#D1D1D1">
                <td width="20%" id="promptMessageId" class="status-font">返回信息:
                    <strong>
                        <font color="red" id="generating">
...
<xsl:template name="op_button_list_gen">
	...
	<!-- 20120411 sam对照表 提交至清算sam表 luojun -->
	<xsl:param name="submitSam"  select="0"/>
	<!-- 20120924 借票归还明细 控制修改车票ID luojun -->
	<xsl:param name="updateId"  select="0"/>
	...
	<xsl:param name="clickMethodVisable" select="''"/>
        ...
        <table width="95%" align="center">
            ...
            <tr>
                <td>
                    ...
                    <xsl:if test="$add=1"><input type="button" id="btAdd" name="add" value="增加" disabled="true0" class="buttonStyle" onclick="{$addBeforeMethod};{$clickMethod};{$addAfterMethod}" /></xsl:if>
                    <xsl:if test="$del=1"><input type="button" id="btDelete" name="del" disabled="true0" value="删除" class="buttonStyle" onclick="{$addQueryMethod};{$clickMethod}" /></xsl:if>
                    ...
                    <xsl:if test="$audit"><input type="button" id="btAudit" name="audit" disabled="true" value="审核" class="buttonStyle" onclick="{$addClickMethod};{$clickMethod};" /></xsl:if> 
                    ...
                    <xsl:if test="$save1=1"><input type="button" id="btSave1" name="save1" disabled="true" value="保存" class="buttonStyle" onclick="{$addQueryMethod};{$clickMethod};{$addClickMethod}" /></xsl:if>
                    <xsl:if test="$submitSam=1"><input type="button" id="btSubmitSam" name="submitSam" disabled="true" value="清算提交" class="buttonStyle" onclick="{$clickMethod}" /></xsl:if>
                    <xsl:if test="$updateId=1"><input type="button" id="btUpdateId" name="updateId" disabled="true" value="拆分车票ID" class="buttonStyle" onclick="{$clickMethodVisable}" /></xsl:if>
                    ...
                    <xsl:if test="$print=1"><input type="button" id="btPrint" name="print" disabled="true" value="打印" class="buttonStyle" onclick="{$clickMethod}" /></xsl:if>
                    ...
                    <xsl:if test="$export1=1"><input type="button" id="btExport1" name="export1" disabled="false" value="导出" class="buttonStyle" onclick="{$clickMethod}" /></xsl:if>
...
<xsl:template name="option_values_limit">
	...
	<xsl:param name="filter_4" select="''"/>
		<input type="hidden" id="{$inputName}" name="{$inputName}">
		  	<xsl:attribute name="value">
		  		<xsl:for-each select="$root">
		  			<xsl:if test="(code =$filter_1)or (code =$filter_2) or (code =$filter_3) or (code =$filter_4)">  
		  				<xsl:value-of select="concat(strType,',',code,',',codeText,':')" />
		  			</xsl:if>
		  		</xsl:for-each>
		    </xsl:attribute>
