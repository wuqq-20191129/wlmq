 ...
 */
//手机支付发售
public class FileRecordParserMobile51 extends FileRecordParserBase{
     @Override
    public Object parse(String line, FileRecordAddVo lineAdd) throws RecordParseForFileException {
        FileRecordMobile51 r = new FileRecordMobile51();
    ...
    private void setCheckInfo(FileRecord51 r) {               
    ...
    private String getCheckUniqueKey(FileRecord51 r){
         //唯一性校验主健
    ...
    private FileRecord00DetailResult getInfoForDeviceCard(FileRecord51 r, char[] b, int offset) throws Exception {
        FileRecord00DetailResult result = new FileRecord00DetailResult();

        int len = 2;
       // r.setTrdType(this.getCharString(b, offset, len));//1交易类型
        r.setTrdType(this.getTrdTypeForMobile(this.getCharString(b, offset, len)));//1手机支付交易类型
        offset += len;
        ...
        len = 20;
        r.setCardLogicalId(this.getCharString(b, offset, len));//6票卡逻辑卡号
        offset += len;

        len = 20;
        r.setCardPhysicalId(this.getCharString(b, offset, len));//7票卡物理卡号
        offset += len;
    ...
    private FileRecord00DetailResult getInfoForTrade(FileRecordMobile51 r, char[] b, int offset) throws Exception {
        FileRecord00DetailResult result = new FileRecord00DetailResult();

        int len = 16;
        r.setSamLogicalId(this.getCharString(b, offset, len));//10SAM卡逻辑卡号
        offset += len;
        ...
        offset += len;
//modify by hejj 20191031 2->4
        len = 4;
        r.setDepositFee(this.getLong(b, offset));//13金额
        ...
        len = 11;
        r.setMobileNo(this.getCharString(b, offset, len));//20手机号码
        offset += len;
        
        len = 2;
        r.setPaidChannelType(this.getCharString(b, offset, len));//21支付渠道
        offset += len;
        
        len = 4;
        r.setPaidChannelCode(this.getCharString(b, offset, len));//22支付渠道代码
        offset += len;
