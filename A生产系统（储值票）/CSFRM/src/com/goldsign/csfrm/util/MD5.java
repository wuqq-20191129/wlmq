...
public class MD5 {
    /*
     * 下面这些S11-S44实际上是䶿䶿4*4的矩阵，在原始的C实现中是璿#define 实现的， 这里把它们实现成为static
     * final是表示了只读，切能在同一个进程空间内的多䶿 Instance间共丿
     */
    ...
    /*
     * 下面的三个成员是MD5计算过程中用到的3个核心数据，在原始的C实现䶿 被定义到MD5_CTX结构䶿
     *
     */
    ...
    /*
     * digestHexStr是MD5的唯䶿䶿个公共成员，是最新一次计算结果的 16进制ASCII表示.
     */
    public String digestHexStr;

    /*
     * digest,是最新一次计算结果的2进制内部表示，表碿128bit的MD5便.
     */
    private byte[] digest = new byte[16];

    /*
     * getMD5ofStr是类MD5暿主要的公共方法，入口参数是你想要进行MD5变换的字符串
    ...
    // 这是MD5这个类的标准构翠函数，JavaBean要求有一个public的并且没有参数的构翠函擿
    ...
    /*
     * F, G, H ,I 斿4个基本的MD5函数，在原始的MD5的C实现中，由于它们斿
     * 笿单的位运算，可能出于效率的濃虑把它们实现成了宏，在java中，我们把它乿 实现成了private方法，名字保持了原来C中的⾿
     */
    ...
    /*
     * FF,GG,HH和II将调用F,G,H,I进行近一步变拿 FF, GG, HH, and II transformations for
     ...
     * 函数由getMD5ofStr调用，调用之前需要调用md5init，因此把它设计成private瘿
     */
    ...
    /*
     * md5Memcpy是一个内部使用的byte数组的块拷贝函数，从input的inpos庿始把len长度瘿
     * 字节拷贝到output的outpos位置庿奿
     */
    ...
    /*
     * Encode把long数组按顺序拆成byte数组，因为java的long类型斿64bit的， 只拆仿32bit，以适应原始C实现的用辿
     */
    ...
    /*
     * Decode把byte数组按顺序合成成long数组，因为java的long类型斿64bit的，
     * 只合成低32bit，高32bit清零，以适应原始C实现的用辿
     */
    ...
    /*
     * b2iu是我写的䶿个把byte按照不濃虑正负号的原则的＂升位＂程序，因为java没有unsigned运算
     */
    ...
    /*
     * byteHEX()，用来把䶿个byte类型的数转换成十六进制的ASCII表示ﺿ
...
//			System.out.println("MD5(\"umps_#24#99#109\"):" + m.getMD5ofStr("umps_#24#99#109"));
//			System.out.println("MD5(\"888888\"):" + m.getMD5ofStr("888888"));
//			System.out.println("MD5(\"88888888\"):" + m.getMD5ofStr("88888888"));
//			System.out.println("MD5(\"message digest\"):"
//					+ m.getMD5ofStr("message digest"));
//			System.out.println("MD5(\"abcdefghijklmnopqrstuvwxyz\"):"
//					+ m.getMD5ofStr("abcdefghijklmnopqrstuvwxyz"));
//			System.out.println("MD5(\"umps@))*\"):"
//					+ m.getMD5ofStr("umps@))*"));
//			System.out.println("MD5(f䷿):"
//					+ m.getMD5ofStr("ukdfjkkfdkfj()*^^%^*&*())121323"));
//			System.out.println("MD5(大幅度空间法扩军):"
//					+ m.getMD5ofStr("大幅度j()*^^%^*&反对汿*())121323"));
//
//			System.out
...
//			System.out.println("MD5(\"RFSIMMOBILEPAY\"):" + m.getMD5ofStr("RFSIMMOBILEPAY").toUpperCase());
//			System.out.println(m.getMD5ofStr("0007        4318125900086101190000008621218CCA77804D2BA1922C33E01511052009041500000003                                0000000001200904151338461"));
//		} else
//			System.out
//					.println("MD5(" + args[0] + ")=" + m.getMD5ofStr(args[0]));
//

        System.out.println(m.getMD5ofStr("0010        3617822200010300001560081021218CCA77804D2BA1922C33E01511052009102900000001010300001636517700000000102009102917051809001"));
    }
}