...
//        alert('checkBoxes'+checkBoxes[i].flag);
...
function selectItemsNoRepeatForDialog(divName, formName) {
    //alert('a');
    var btId = event.srcElement.id;
    if (btId == 'cancel') {
    ...
    getAllSelectedIDsForNoRepeat(divName, formName);
    var allSelectedIds = document.forms[formName].getElementsByTagName('input')['allSelectedIDs'].value;
//    alert('allSelectedIds:'+allSelectedIds);
    window.returnValue = allSelectedIds;
    window.close();
    return;
}
...
//        alert(validDateOb.require);
    ...
    if ((areaIdOb.value == '01') ||
        ...
        //面值、有效期、逻辑卡号、逻辑卡号复选框不可见
        //add by zhongzq 20170920
        disableControls([validDateOb],true);
        disableControls([startLogicalIdOb, endLogicalIdOb, selectOb[1]], true);
        //数量校验
        validControls([outNumOb], 'true');
    ...
    //赋值区、储值票编码区出库，按数量或逻辑卡号
        ...
        //逻辑卡号不校验
        //add by zhongzq 20170920
        disableControls([validDateOb],false);
        validControls([startLogicalIdOb, endLogicalIdOb], 'false');
        //数量复选框选中
        selectOb[0].checked = true;
...
//    var cardMoneyOb = frm.getElementsByTagName('select')['d_cardMoney'];
    ...
    var validDateOb = frm.getElementsByTagName('input')['d_validDate'];
//     alert('validTest.type='+validTest.type)
//    var validDateOb1 = frm.getElementsByTagName('input')['d_validDateSelect'];
    var selectOb = frm.getElementsByTagName('input')['d_select'];
...
//        disableControls([startLogicalIdOb, endLogicalIdOb, boxIdOb, endBoxIdOb, selectOb[1], selectOb[2]], true);//增加日期 20171030 zhongzq
        disableControls([validDateOb,startLogicalIdOb, endLogicalIdOb, boxIdOb, endBoxIdOb, selectOb[1], selectOb[2]], true);
        //数量校验
        validControls([outNumOb], 'true');
        //逻辑卡号、盒号不校验
    ...
    //赋值区、储值票编码区出库，按数量或逻辑卡号、盒号，默认：按数量
        ...
        //数量，逻辑卡号、盒号复选框可见
        //disableControls([cardMoneyOb,cardMoneyOb1,validDateOb,outNumOb,selectOb[1]],false);
//        disableControls([outNumOb, selectOb[1], selectOb[2]], false);//增加日期 20171030 zhongzq
        disableControls([validDateOb,outNumOb, selectOb[1], selectOb[2]], false);
        //数量、面值校验
...
//add by zhonzgq 20171101
function clearNumForDistributeLine(fromName){
    var frm = document.forms[fromName];
    var outNumOb = frm.getElementsByTagName('input')['d_distributeQuantity'];
    var selectOb = frm.getElementsByTagName('input')['d_select'];
    var reasonIdOb = frm.getElementsByTagName('select')['d_reasonId'];
     var selectedOb = reasonIdOb.options[reasonIdOb.selectedIndex];
    if (selectedOb.value == '17') {
        selectOb[0].checked = false;
        selectOb[0].disabled = true;
         outNumOb.value = "";
    }
}
...
//        controls[i].require = 'true';
        controls[i].setAttribute("require",'true');
    }
}
...
function setUnavailableControls(controls) {
    for (var i = 0; i < controls.length; i++) {
        ...
        controls[i].setAttribute("require",'false');
...
//20180315 mqf 进出站限制模式在“预赋值”可用，不限制票卡类型
function controlDisableForMode(formName, workTypeName, controlNames) {
    var frm = document.forms[formName];
    var workType = frm.getElementsByTagName('select')[workTypeName].value;
//    var cardMainType = frm.getElementsByTagName('select')[cardMainTypeName].value;
    // alert('worktype='+workType+' cardSubType='+cardSubType);
...
function controlByCardTypeAndAreaForMode() {
    ...
    //多日票、赋值区
//    if ((icMainTypeOb.value == '40' && areaIdOb.value == '03')) {//modify by zhongzq 20171030 赋值区均可限制
...
//add by zhongzq 20170804 只有赋值区才有面值其他都是0
...
//                alert("icCardMoney1.options[i].selected:"+i);
        ...
        disableControls([icCardMoney,icCardMoney1],false);
//        icCardMoney.disabled = true;
//        icCardMoney1.disabled = true;
    } else {
         disableControls([icCardMoney,icCardMoney1],true);
...
function controlByCardTypeAndAreaForModeForBorrow() {
    ...
    //多日票、赋值区
//    alert(areaIdOb.value)
//    if (areaIdOb.value == '03'&&icMainTypeOb.value == '40') {//modify by zhongzq 20170920
    if (areaIdOb.value == '03') {
        modeOb.disabled = false;
    } else {
        modeOb.value = '000';
...
function controlCardMoneyWorkType(formName, workTypeName, cardMoney, cardMoney1) {
    ...
    //20180315 mqf 加上 注销 '03'
    if (obWorkType.value == '00' || obWorkType.value == '02' || obWorkType.value == '03') {
...
//    if (obWorkType.value == '01' || obWorkType.value == '03') {
    //20180412 obWorkType: '01' 预赋值 改为可以选择“记名卡标志”，ES生产系统需要对该字段进行生产控制
    if (obWorkType.value == '03') {
...
//        alert("icMainType:"+icMainType.value+"icSubType:"+icSubType.value+"areaId:"+areaId.value);
    ...
    if (areaId.value == '02' || areaId.value == '03') {
        ...
        if (icMainType.value == '1' || icMainType.value == '2' || icMainType.value == '40'
                || icMainType.value == '7' || icMainType.value == '8' || icMainType.value == '10') {
            flag = 1;
        }
    }
    if (flag == 0) {
...
function controlValidDateByWorkType(formName, workTypeName, validDate, lblValidDate) {
    var frm = document.forms[formName];
    var obWorkType = frm.getElementsByTagName('select')[workTypeName];
    var obValidDate = frm.getElementsByTagName('input')[validDate];
    var obLblValidDate = frm.getElementsByTagName('td')[lblValidDate];
    if (obWorkType.value == '01' || obWorkType.value == '03') { //预赋值、注销
        obValidDate.value = "";
//        obLblValidDate.innerText = "逻辑有效期";
        obValidDate.disabled = true;
//    } else if (obWorkType.value == '03') { //注销

    } else { //初始化、重编码
        obValidDate.disabled = false;
        if (obValidDate.value == null || obValidDate.value == "") {
            var cur = new Date();
            var miliValue = cur.getTime();
            var miliValid = miliValue + 1 * 24 * 3600 * 1000; //默认第二天
            obValidDate.value = getFormatDate(new Date(miliValid));
//            obLblValidDate.innerText = "物理有效期";
        }
    }

}
//通过出库原因控制配发线路控件
function controlDistributeLineByReason(formName,reasonName,lineName){
	var frm = document.forms[formName];
    var reaonOb = frm.getElementsByTagName('select')[reasonName];
	frm.getElementsByTagName('select')[lineName].disabled = true;
    frm.getElementsByTagName('select')[lineName].require = 'false';
//    frm.getElementsByTagName('select')[lineName].value = '';
    //只有配票到其他部门可供选择
    if (reaonOb.value == '18') {
    	frm.getElementsByTagName('select')[lineName].disabled = false;
    }
}

//20200901 mqf 票库迁移，根据票区控制面值选择框、输入框
function controlMoveCardMoney(formName, areaIdName, cardMoneyName, cardMoney1Name) {

    var frm = document.forms[formName];
    var areaId = frm.getElementsByTagName('select')[areaIdName];
    var cardMoney = frm.getElementsByTagName('input')[cardMoneyName];
    var cardMoney1 = frm.getElementsByTagName('select')[cardMoney1Name];
    var flag = 0;
    //赋值区的票卡才有面值
    if (areaId.value == '03') {
        flag = 1;
    ...
    if (flag == 0) {
        ...
        setUnavailableControls([cardMoney, cardMoney1]);
        cardMoney.value = '0';
        cardMoney1.value = '000';

    } else {

        setAvailableControls([cardMoney, cardMoney1]);
