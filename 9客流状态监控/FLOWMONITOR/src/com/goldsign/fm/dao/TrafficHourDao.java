 ...
 */
public class TrafficHourDao {
    private static Logger logger =Logger.getLogger(TrafficHourDao.class.getName());

    public Vector getLineStationSpanTraffic(String clickNode, String currentTrafficDate) throws Exception {
        ...
        String[] fieldsAllLine = {"hour", "traffic", "memo"};
        String[] fieldsSingleLine = {"line_id", "hour", "traffic", "memo"};
        String[] fieldsStation = {"line_id", "station_id", "hour", "traffic", "memo"};

        try {
            dbHelper = new DbHelper("",AppConstant.dbcpHelper2.getConnection());
            strSql = "{call w_acc_mn.W_UP_FM_FLOW_HOUR (?,?,?) }";
    ...
    private DrawOriginResult getListSpanItem(DbHelper dbHelper, String[] fields, String clickNode) throws SQLException {
        ...
        for (int i = 0; i < fields.length; i++) {
            ...
            if (field.equals("hour")) {
                hour = dbHelper.getItemIntValue("hour");
                continue;
            }
