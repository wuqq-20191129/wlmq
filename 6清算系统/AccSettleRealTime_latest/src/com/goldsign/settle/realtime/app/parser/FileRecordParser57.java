 ...
 */
////退卡记录（即时、非即时的退款结果）
public class FileRecordParser57 extends FileRecordParserBase {
    //0：即时退款；1：非即时退款；2：单程票退款；3：非正常模式单程票退款。

    //private static String RETURN_TYPE_NON_INSTANT = "1";//非即时退款
    //private static String RETURN_TYPE_INSTANT = "0";//即时退款

    @Override
    public Object parse(String line, FileRecordAddVo lineAdd) throws RecordParseForFileException {
        FileRecord57 r = new FileRecord57();
    ...
    private FileRecord00DetailResult getInfoForDevice(FileRecord57 r, char[] b, int offset) throws Exception {
    ...
    private FileRecord00DetailResult getInfoForTrade(FileRecord57 r, char[] b, int offset) throws Exception {
        FileRecord00DetailResult result = new FileRecord00DetailResult();
