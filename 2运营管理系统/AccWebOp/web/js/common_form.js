...
function getAvriableOpertions(formName, opCode) {
    ...
    if (opCode != "") {
        ...
        // alert("opCode="+opCode+"
		// avariableOpertions.avriable="+avariableOpertions.avriable);
...
function clickResultRow(formName, object, detailFrom) {
    ...
    // alert("left="+headEventObject.offsetLeft+"
	// top="+headEventObject.offsetTop);
    ...
    if (oldLine != "") {
        ...
        oldObject.style.background = rowBackGround;// #FFFFFF";
    }
    // 设置当前行
            ...
            if (currentLine != "") {
                ...
                if (tmp != 0) {
                    for (var i = 0; i < tmp; i++) {
                        ...
                        // alert("child.id=" + child.id + " detail=" + detail.id
						// + " tmpId=" + "d_" + child.id);

                        /** ****** 设置detailForm ********* */
                        if (child.id != "") {
                            // alert(child.id + " to find " + "d_" + child.id);
                            var tmpObj = document.getElementById("d_" + child.id);// 明细输入框id加前缀
																					// d_
                                        ...
                                        switch (tmpObj.type) {
                                            ...
                                            case "radio":
                                            {
                                                ...
                                                for (var k = 0; k < radioGroup.length; k++) {
                                                    ...
                                                    {
                                                        // radioGroupChild.defaultChecked
														// = true;
                                    ...
                                    case "SELECT":
                                    {
                                        ...
                                        if (tmpObj.type == "select-one") {
                                            for (j = 0; j < tmpObj.options.length; j++) {
                                                // alert("="+tmpObj.options[j].value+"=
												// =" +
												// tmpObj.options[j].text+"= ="
												// +child.innerHTML+"=" +
												// (tmpObj.options[j].value ==
												// child.innerHTML ||
												// tmpObj.options[j].text
												// ==child.innerHTML));
                                        ...
                                        if (tmpObj.type == "select-multiple") {
                                            for (j = 0; j < tmpObj.options.length; j++) {
                                                ...
                                                var idx1 = tmpObj.options[j].value==""?-1:child.innerHTML.indexOf(tmpObj.options[j].value);
                            ...
                            // --del tmpObj = document.getElementById("d1_" +
							// child.id);//明细输入框id加前缀 d_
                            /*--del
                             if (tmpObj != null && tmpObj != "undefined") {
                             switch (tmpObj.tagName) {
                             case "INPUT":
                ...
                if (tmp != 0) {
                    for (var i = 0; i < tmp; i++) {
                        ...
                        if (child.id != "") {
                            ...
                            tmpObj = document.getElementById("d1_" + child.id);// 明细输入框id加前缀
																				// d_
                            if (tmpObj != null && tmpObj != "undefined") {
                                switch (tmpObj.tagName) {
                                    case "INPUT":
...
// 全选按钮
/*
 * function selectAllRecord(formName,headCheckName,name){ var
 * currentOperation=getCurrentOpertion(formName); if (currentOperation ==
 * "op_modify" || currentOperation == "op_add") return false;
 *
 * var obj=document.children[headCheckName]; var
 * count=document.forms[formName].children["rowTickedCount"];
 *
 * var flag=false; var temp=0;
 *
 * for (var i=0;i<document.all.length ;i++ ) { var e=document.children[i];
 *
 * if (e.name==name && e.type=="checkbox") { if (obj.checked){ e.checked="1";
 * flag=true; } else{ e.checked=""; temp=0; } if (flag) temp++; } }
 * count.value=temp;
 *
 * //改变操作模式（改变按钮的可用状态） if (flag) if (getCurrentLine(formName) != "")
 * setOperation(formName,"op_select"); else
 * setOperation(formName,"op_select_all"); else if (getCurrentLine(formName) !=
 * "") setOperation(formName,"op_select_browse"); else
...
// alert('chileren=' +
// headEventObject.children.length+':headEventObject.id='+headEventObject.id);
// 改变点击列的类风格
    ...
    for (i = 0; i < trObject.length; i++) {
        if (trObject[i].id != 'ignore') {
            ...
            if (trObject[i].children[colNum] == null || trObject[i].children[colNum].innerHTML == null)
                ...
                tdinnerHTML[i0] = trObject[i].children[colNum - 1].innerHTML.trim(); // 修改排序错误问题
																						// modify
																						// by
																						// lucankun
																						// 20170220
...
// formName
// 提交窗体名称；reportType报表类型；controlNames查找报表代码的控件名称按顺序用＃分隔;tableNamex显示列表名称
...
// var dateOb = document.forms[formName].children['date'];
    // 20171031 moqf
    var dateOb = document.forms[formName].getElementsByTagName("input")['date'];
    if (dateOb == null)
        return false;
    ...
    return true;
    /*
	 * var fleName = date+"-"+rptCode+".pdf"; var caption = dateOb.value;
...
function setLinkText(formName, reportType, controlNames) {
    ...
    var tableName = "reportList";
    /*
	 * switch(reportType){ case "1" :{ var yearOb = frm.children['year'];
	 * caption = yearOb.value; break; } case "2" :{ var yearOb =
	 * frm.children['year']; var monthOb = frm.children['month']; caption =
	 * yearOb.value+'-'+monthOb.value; break; } case "3" :{ var dateOb =
	 * frm.children['date']; caption = dateOb.value; break; } }
	 */
    caption = addCaptionInfo(formName, controlNames, caption);
...
// var monthOb = document.forms[formName].children['month'];
    var yearOb = document.forms[formName].getElementsByTagName("select")['year'];
    var monthOb = document.forms[formName].getElementsByTagName("select")['month'];
    ...
    /*
	 * if(isBalanceDate(formName)) { balanceDateOb
	 * =document.forms[formName].children['balanceDate']; balanceDate
	 * =balanceDateOb.value; if(checkBalanceDate(balanceDate)==false) return
	 * false;
	 *
	 *
	 * balanceDate =
	 * balanceDate.substring(0,4)+balanceDate.substring(5,7)+balanceDate.substring(8,10);
	 * fleName = balanceDate+"."+rptCode; }
	 */
    setReportNamePrefix(formName, fleName);
    return true;
    /*
	 * var fleName = month+"-"+rptCode+".pdf"; var caption =
	 * yearOb.value+"-"+monthOb.value; caption =
	 * addCaptionInfo(formName,controlNames,caption);
	 * displayReport(tableName,fleName,caption);
	 */
...
// selYear = frm.children['year'].value;
            // 20171031 moqf
            selYear = frm.getElementsByTagName("select")['year'].value;
            if (selYear > sYear) {
                alert("选择的年份不能大于当前年");
                return false;
            }
...
// selMonth = frm.children['month'].value;
            // 20171031 moqf
            selYear = frm.getElementsByTagName("select")['year'].value;
            selMonth = frm.getElementsByTagName("select")['month'].value;
...
// selDate = frm.children['date'].value;
            // 20171031 mqf
            selDate = frm.getElementsByTagName("input")['date'].value;
            if (selDate > sDate) {
                alert("选择的日期不能大于当前日期");
                return false;
            }
...
// var yearOb = document.forms[formName].children['year'];
    ...
    return true;
    /*
	 * var fleName = year+"-"+rptCode+".pdf"; var caption = yearOb.value;
	 * caption = addCaptionInfo(formName,controlNames,caption);
	 * displayReport(tableName,fleName,caption);
	 */
...
// 20171107 moqf
        ob = document.getElementById(controlNamesArray[i]);
        if (ob == null)
            continue;
        key += ob.value;
    }
...
// var rptCodeMappingOb = frm.children['reportCodeMapping'];
    // 20171107 moqf
    var rptCodeMappingOb = frm.getElementsByTagName("input")['reportCodeMapping'];
...
// alert("btExport");
        ...
        case "btInBlack":
        {
            var rst = confirm("确定黑名单提交？");
            if (!rst)
                return;
            setCommand(formName, "inBlack");
            setSubmitForm(formName);
            submitSamAction(formName);
            break;
        }
...
// alert("call btClone");
        ...
        case "btResetPass":
        {//重置密码
            var rst = confirm("确定重置密码？");
            if (!rst)
                return;
            setCommand(formName, "passReset");
             setOperation(formName, "op_modify");
           setSubmitForm(formName);
           saveAction(formName);
            break;
...
// alert("btExport");
        ...
        {//跳转
            //校验输入的页码
            if(!checkGoPageNos(formName)){
                alert("跳转页码应为正整数！");
                return;
            }
            setCommand(formName, "goPage");
            setOperation(formName, "op_next");
            pageControlAction(formName);
            break;
        }
        ...
        case "btGenFile":
        {
            var rst = confirm("确定对所选记录生成文件？");
            if (!rst)
                return;
            setCommand(formName, "genFile");
            setOperation(formName, "op_genFile");
            setSubmitForm(formName);
            getAllSelectedIDs(listName, formName);
            genFileAction(formName);
            break;
        }
         case "btInBlack":
        {
...
//           var id = document.getElementById('spinnerTip');
//           disabeQueryControl(formName, "expAll");
           //alert("");
//           window.location.reload();
           setCommand(formName, "expAll");
//           alert (filePath==="");
//           var filePath =  ajaxMethod();
             exportAll(formName);
//           var expFileName = document.getElementById('expFileName');
//           var form=$("<form>");// 定义form表单,通过表单发送请求
//           form.attr("style","display:none");// 设置为不显示
//           form.attr("target","");
//           form.attr("method","post");// 设置请求类型
//           form.attr("action","Download");// 设置请求路径
//           $("body").append(form);// 添加表单到页面(body)中
//           var inputFilePath=$("<input>");
//           inputFilePath.attr("type","hidden");// 设置为隐藏域
//           inputFilePath.attr("name","filePath");// 设置参数名称
//           inputFilePath.attr("id","filePath");// 设置参数id
//           inputFilePath.attr("value",filePath);// 设置参数值
//           var inputExpFileName=$("<input>");
//           inputExpFileName.attr("type","hidden");// 设置为隐藏域
//           inputExpFileName.attr("name","expFileName");// 设置参数名称
//           inputExpFileName.attr("id","expFileName");// 设置参数id
//           inputExpFileName.attr("value",expFileName.value);// 设置参数值
//           form.append(inputFilePath);// 添加到表单中]
//           form.append(inputExpFileName);// 添加到表单中
//           form.submit();// 表单提交、
//           $(form).remove();
//           deleteSpinner();

//            setSubmitForm(formName);
//            submitExpAllAction(formName);
            break;
        }
...
/*
 * 导出单据 formName:窗体名称 template：模板名称 paramNames：参数名称 paramValueNames：参数值的控件名称
 * iWidth:宽 iHeight：高
 */
...
function checkForStatistic(formName) {
    ...
    // alert("selIndex="+selIndex+" relIndex="+relIndex+"
	// relIndex1="+relIndex1+" relIndex2="+relIndex2);
    if (selIndex != -1) {
...
// var rt = window.showModalDialog("jsp/component/futureVersion.jsp",
// currentVersionStartDate,
// "center:true;status:no;dialogWidth:250px;dialogHeight:150px;help:no");
...
function importAction(formName) {
    // <input type="hidden" name="importUrl" id="importUrl"
	// value="phyLogicList?command=import" />
    // 其中 phyLogicList?command=import 打开导入的页面
    var importUrl = document.forms[formName].getElementsByTagName("input")['importUrl'].value;
    if(importUrl == null || importUrl == 'undefined'|| importUrl == ''){
        window.open("./fileUpload.jsp", "", "width=600,height=300,left=360,top=310");
    }else{
        var iWidth=730;                          // ڵĿ ;
        var iHeight=430;                        // ڵĸ߶ ;
        var iTop = (window.screen.availHeight-30-iHeight)/2;       // ô ڵĴ ֱλ ;
        var iLeft = (window.screen.availWidth-10-iWidth)/2;           // ô ڵ
																		// ˮƽλ ;
...
// }
// 取消时，恢复斑马线原始颜色 zhouy 20171218
    var obTable = document.getElementById('DataTable');
    if (obTable.children[0] == null)
        return;
    var obRows = obTable.children[0].children;
    var obRow;
...
/*
 * function setDetailFormEnabled(formName,flag){ if(formName == '') return; var
 * detail=document.forms[formName]; if (detail != null){ var
 * detailTable=detail.children[0];//table while (detailTable.children[0].tagName !=
 * "TR"){ detailTable=detailTable.children[0]; }
 *
 * for (var i=0;i<detailTable.children.length;i++){ var
 * detailTR=detailTable.children[i];//TR for (var j=0;j<detailTR.children.length;j++){
 * var detailTD=detailTR.children[j]; //TD var tmpLen=detailTD.children.length;
 * if (tmpLen>0){ for (var jj=0;jj<tmpLen;jj++){
 * //alert(detailTD.children[jj].name + " " + detailTD.children[jj].disabled);
 * if (flag) detailTD.children[jj].disabled=""; else
 * detailTD.children[jj].disabled="true"; }//for jj }//if }//for j }//for i
 * }//if }
 */
function setDetailFormEnabled(formName, flag) {
    var detail = document.getElementById(formName);
    if(detail===null){ // add by zhongziqi 20170614
        return ;
    }
...
function showListItems_1(listDivName, colNum, ob) {
// alert("showListItems");
    var listDiv = document.getElementById(listDivName);
    if (listDiv.tagName != 'DIV')
        return false;
    ...
    var obTable = listDiv.children[0];
    var obTBody = obTable.children[0];
    // alert('isDigit='+isDigit);
    if (obTBody == null)
        return;
    ...
    for (i = 0; i < obRows.length; i++) {
        ...
        var dataRow = obRows[i];
        // alert("dataRow.cells[colNum].innerText="+dataRow.cells[colNum].innerText);
        // alert("key="+key);
        cellText = dataRow.title;
// cellText = dataRow.cells[colNum].innerText;
        idx = cellText.indexOf(key);
        if (idx == -1)
            dataRow.style.display = 'none';
...
// add by zhongziqi 20170615
function disabledById(fromName,disableId){
    var cmd = document.forms[fromName].all['command'].value;
    if(cmd =="modify"){
...
// modify by hejj 20110528
        /*
		 * if(document.forms[formName].children["command"].value =="modify")
		 * enablePrimaryKeys(formName); setAlwaysDisableForObj(formName,false);
		 *
		 * disabeQueryControl(formName,'save');
		 */
        enablePrimaryKeys(formName); // 改放在校验成功后
...
function submitExpAllAction(formName) {
    var divHeadOb = document.getElementById('clearStartHead');

    var tableHead = divHeadOb.children[0];
    var expAllTableHeadName = document.getElementById('expAllTableHeadName');
    var expAllTableHeadWidth = document.getElementById('expAllTableHeadWidth');

    expAllTableHeadName.value="";
    expAllTableHeadWidth.value="";
    // table.rows[i].cells[j].children[0].type == "checkbox"
    for(i = 0 ; i<tableHead.rows[0].cells.length;i++){
        // alert(tableHead.rows[0].cells[i].children[0].type);
        if(typeof(tableHead.rows[0].cells[i].children[0]) == "undefined"){

            expAllTableHeadName.value += tableHead.rows[0].cells[i].innerText
            expAllTableHeadName.value += ",";
            expAllTableHeadWidth.value += tableHead.rows[0].cells[i].style.width;
            expAllTableHeadWidth.value += ",";
        }

    }
// alert(expAllTableHeadWidth.value);
// alert(expAllTableHeadName.value);
// alert(tableHead.rows[0].cells[1].innerText);
    frm = document.forms[formName];
    // add by zhongzq 20180129
// if (!Validator.Validate(frm, '1'))
// return;
    disabeQueryControl(formName, "expAll");
    // alert('disabeQueryControl');
...
// alert(document.getElementById(objName));
        if (document.getElementById(objName) != null)
            ...
            document.getElementById(objName).disabled = flag;
    }
    objs = objNames.split(',');
...
// alert('paramTypeValue='+paramTypeValue+"
// versionTypeValue="+versionTypeValue+" versionObj="+versionObj);
    if (versionObj != "") {
// alert('setversion0');
...
// 换乘车站 明细 车站2
function setSelectValuesTSC(formName, selectedName, selectName, varName, selectStationOne) {
// alert("formName="+formName);
// alert("varName="+varName);
// alert("varValues="+document.forms[formName].children[varName].value);
    var cmdV = document.forms[formName].getElementsByTagName("input")[varName];
    var sltd = document.forms[formName].getElementsByTagName("select")[selectedName];
    var slt = document.forms[formName].getElementsByTagName("select")[selectName];
    var sltOne = document.forms[formName].getElementsByTagName("select")[selectStationOne];
    var sltOneObj = document.getElementById(selectStationOne);
    var index = sltOneObj.selectedIndex; // 选中索引
    var sltOneText = sltOneObj.options[index].text; // 选中文本
    var typ = sltd.value;
    if (cmdV.value == null || cmdV.value == '')
        return;
    if (cmdV.value.indexOf(":") == -1)
    ...
    for (i = 0; i < values.length; i++) {
        ...
        if (objs[0] == typ) {
            // alert("osltOneText:" + sltOneText + ";objs[2]:" + objs[2] +
			// ";objs[1]:" + objs[1] );
            if(objs[2] == sltOneText){
                var opt = new Option(objs[2], objs[1]);
                slt.add(opt);
                if (count == 0)
                    opt.selected = true;
...
// oSheet.Application.CentimetersToPoints(sideMargin);
...
//     alert("priviCtr.value="+priviCtr.value);
    ...
    var privi = "";
    var result = "";
//    alert("privis:"+privis+"|| privis.length:"+ privis.length);
    for (i = 0; i < privis.length; i++) {
        ...
        if (privi == "1") {
            ...
            if (i == 19) {
                result += getPriviAvailButtonList(formName, "genFile") + ",";
                continue;
            }
             if (i == 20) {
                result += getPriviAvailButtonList(formName, "inBlack") + ",";
                continue;
            }
             if (i == 21) {
                 result += getPriviAvailButtonList(formName, "expAll") + ",";
//                 alert("result:"+result);
                 continue;
             }
...
function getPriviAvailButtonList(formName, priviID) {
    ...
    if (priviID == "genFile")
        return frm.children['priviGenFile'].getAttribute('value');
    if (priviID == "inBlack")
        return frm.children['priviInBlack'].getAttribute('value');
    if (priviID == "expAll")
        return frm.children['priviExportAll'].getAttribute('value');
}
...
function isPriviAvailButton(privAvail, availButton) {
    if (privAvail == "" || availButton == "" || privAvail == undefined || privAvail == "undefined" || availButton == "undefined")
        return false;
...
// alert("controlName="+controlName+" defValue="+defValue);
    ...
    // alert("lineName="+lineName+" stationName="+stationName+"
	// mainCardName="+mainCardName+" subCardName="+subCardName);
...
// alert("formName="+formName+" yearName="+yearName+" monthName="+monthName+"
// weekName="+weekName);
    var frm = document.forms[formName];
    ...
    var week_spans = [];
    var week_num = 0;
    // alert(" begin_date="+begin_date.toLocaleString()+"
	// begin_date_day="+begin_date_day+" end_month_day="+end_month_day);
...
// alert("year="+year+" month="+month+" rang="+rang+"
...
// alert("printRows="+printRows);
        ...
        for (j = 0; j < colCount; j++)
        {
            if (i == 0) {
                ...
                if (table.rows[0].cells[j].children[0] == null) {
                    oSheet.Cells(l + 1, k + 1).value = cellText;

                    if(i==0){
                        oSheet.Cells(l + 1, k + 1).Font.Bold = true; // 设置为粗体*
                        sL += 2*cellText.length;
                        oSheet.Cells(l + 1, k + 1).ColumnWidth = 2*cellText.length;
                    }
                    oSheet.Cells(l + 1, k + 1).Borders.Weight = 2; // 设置单元格边框*()
                    oSheet.Cells(l + 1, k + 1).Font.Size = 10;
//                    oSheet.Columns.AutoFit(); // 自动列宽
    ...
    if(sL <= 30){
        oSheet.PageSetup.LeftMargin = 8/0.035;
    }
    else if(sL < 40){
        oSheet.PageSetup.LeftMargin = 7.5/0.035;
    }
     else if(sL <= 50){
        oSheet.PageSetup.LeftMargin = 6.5/0.035;
    }
    else if(sL <= 60){
        oSheet.PageSetup.LeftMargin = 5/0.035;
    }
    else if(sL <= 70){
        oSheet.PageSetup.LeftMargin = 3.5/0.035;
    }
    else if(sL <= 80){
        oSheet.PageSetup.LeftMargin = 2/0.035;
    }
    else if(sL <= 90){
        oSheet.PageSetup.LeftMargin = 5.5/0.035;
    }
    else{
        oSheet.PageSetup.LeftMargin = 4/0.035;
        oSheet.PageSetup.RightMargin = 4/0.035;
    }
    oXL.Visible = true;
    // oXL.UserControl = true;
    return oXL;
...
function setPageControl(formName) {
    ...
    setPageControlForGoPage(formName);
}
...
function setPageControlForGoPage(formName) {
    var oNextEnd = document.forms[formName].getElementsByTagName("input")['_nextEnd'];
    var oBackEnd = document.forms[formName].getElementsByTagName("input")['_backEnd'];
    var oCurrent = document.forms[formName].getElementsByTagName("input")['_current'];
    var sNextEnd = oNextEnd.value;
    var sBackEnd = oBackEnd.value;
    var sCurrent = oCurrent.value;
    var goPage = document.forms[formName].getElementsByTagName("input")['btGoPage'];
    if(goPage != undefined && goPage != null){
        removeClass(goPage, "buttonStyle");
        if ((sNextEnd == null || sNextEnd.length == 0) && (sBackEnd == null || sBackEnd.length == 0)) {
            goPage.disabled = true;
            return;
        }
        var iNextEnd = sNextEnd - 0;
        var iCurrent = sCurrent - 0;
        if (iNextEnd == 1 || iNextEnd == iCurrent) {  //不论是第一页还是最后一页都显示
            addClass(goPage, "buttonStyle");
            goPage.disabled = false;
        } else {
            addClass(goPage, "buttonStyle");
            goPage.disabled = false;
        }
...
function checkGoPageNos(formName){
    var frm = document.forms[formName];
    var goPageNos = frm.getElementsByTagName("input")["goPageNos"];
    if (goPageNos !== null){
        var goPageNosValue = goPageNos.value;
        var r = /^[0-9]*[1-9][0-9]*$/;
        return r.test(goPageNosValue);
    }
}
...
// $("body").append( "<div id='spinnerBG'></div><div id='spinnerTip'><img
// src='"+
// contextPath+"/images/spinner.gif' /></div>" );
// $("body").append( "<div id='spinnerBG'></div><div id='spinnerTip'><img
// src='./images/spinner.gif' /></div>" );
    var objDiv = document.createElement("DIV");
    objDiv.innerHTML = "<div id='spinnerBG'></div><div id='spinnerTip'><img src='./images/spinner.gif' /></div>";
    document.body.appendChild(objDiv);
}
...
// src='./images/spinner.gif' /></div>" );
	    ...
	    var spinnerTip = document.getElementById("spinnerTip");
	    if(spinnerTip){
	    	spinnerTip.parentNode.removeChild(spinnerTip);
	    }
	    if(spinnerBG){
	    	spinnerBG.parentNode.removeChild(spinnerBG);
	    }
	}
...
// 给window返回所选值 20170630 zhouyang
function selectItemsForDialogDevID(divName,formName){
    ...
    var rowSelected =document.forms[formName].getElementsByTagName("input")["rowSelected"].value;

  	window.returnValue=rowSelected;
    window.close();
    return;
}
// 关闭窗口 20170630 zhouyang
...
function genFileAction(formName) {

    frm = document.forms[formName];
    frm.submit();
}
//add by zhongziqi 20180202
function exportAll(formName) {
	//播放动画
	showSpinner("");
	var divId = document
	.forms[formName].getElementsByTagName("input")["divId"];
	var methodUrl = document
	.forms[formName].getElementsByTagName("input")["methodUrl"];

	var divHeadOb = document.getElementById(divId.value);
	var tableHead = divHeadOb.children[0];
	var expAllTableHeadName = document
			.forms[formName].getElementsByTagName("input")["expAllTableHeadName"];
	var expAllTableHeadWidth = document
			.forms[formName].getElementsByTagName("input")["expAllTableHeadWidth"];
	var expAllFields = document.forms[formName].getElementsByTagName("input")["expAllFields"];
	//var templateName = document.getElementById('templateName');
	var expFileName = document.forms[formName].getElementsByTagName("input")["expFileName"];
	expAllTableHeadName.value = "";
	expAllTableHeadWidth.value = "";
	//table.rows[i].cells[j].children[0].type == "checkbox"
	for (i = 0; i < tableHead.rows[0].cells.length; i++) {
	...
	var url = localhostPath + projectName + methodUrl.value;
	...
	xhr.onload = function() {
		...
		if (this.status === 200) {
			...
			var inputFilePath = document.createElement('input');
			inputFilePath.type = 'hidden';
			inputFilePath.name = 'filePath';
			inputFilePath.id = 'filePath';
			inputFilePath.value =json.exportExcelPath;
			var inputExpFileName = document.createElement('input');
			inputExpFileName.type = 'hidden';
			inputExpFileName.name = 'expFileName';
			inputExpFileName.id = 'expFileName';
			inputExpFileName.value = expFileName.value;
			form.appendChild(inputFilePath);// 添加到表单中]
			form.appendChild(inputExpFileName);// 添加到表单中
			document.body.appendChild(form);
			form.submit();
			document.body.removeChild(form);
		} else {
			alert("导出全部失败，请查看后台日志");
		}
		deleteSpinner();
	};
	...
	var para = 'expAllTableHeadName=' + expAllTableHeadName.value
			+ '&expAllTableHeadWidth=' + expAllTableHeadWidth.value
			+ '&expAllFields=' + expAllFields.value + '&expFileName='
			+ expFileName.value;
	//alert(para);
	// 发送ajax请求
	xhr.send(para);
...
//add by zhongzq 20180301 设置当前日期
function setCurDate(formName,id){
	var curDate = document
	.forms[formName].getElementsByTagName("input")[id];
...
function keyDownEvent(obj,obj0,obj1,obj2)
{
    code=event.keyCode;
    ...
    if(code==110||code==190)
        if(obj.value)
            ...
            event.returnValue=false;
    if(obj1.value.length>=3&&code!=37&&code!=39&&code!=16&&code!=9&&code!=13 &&code!==8)
        obj2.focus();
    if(obj1.value.length>=3&& (code==110 || code==190))
        obj1.focus();
    if(code == 32)
        obj2.focus();
...
function keyUpEvent(obj){
    if (obj.value > 255)
    {
        alert(obj.value+"不是有效项，请指定一个介于1和255间的值");
        obj.value = obj.value.substring(0, obj.value.length - 1);
        return;
    }
}

function keyUpEventForIp1(obj){
    if (obj.value > 223)
    {
        alert(obj.value+"不是有效项，请指定一个介于1和223间的值");
        obj.value = obj.value.substring(0, obj.value.length - 1);
        return;
    }
}
...
function getipvalue(obj1,obj2,obj3,obj4,obj){
    document.getElementById(obj).value = document.getElementById(obj1).value + "." + document.getElementById(obj2).value + "." + document.getElementById(obj3).value + "." + document.getElementById(obj4).value;
}

function onclickSetIpValue(ip1,ip2,ip3,ip4,IpValue){
    var arr = [];
    arr = IpValue.split(".");
...
//zhongziqi 20180312
//清理
function removeBlankSpaceBeforeQuery(form, names) {
  if (form == "" || names == "") {
//      alert("heihei");
      return;
  }
//  alert(names.length);
  ...
  for (var i = 0; i < names.length; i++) {
      var name = names[i];
      var input = frm.getElementsByTagName("input")[name];
      input.value = input.value.toString().replace(/\s/g, "");
  }
}
//add by zhongzq 20180425 点击一条记录勾选
function clickOneResultRow(formName, object, detailFrom, name) {
