 ...
 * @author hejj
 */
public class Trade73Dao extends TradeBaseDao{
     private static Logger logger = Logger.getLogger(Trade73Dao.class.getName());
    ...
    public int insertError(FileRecordBase frb,String errCode)  {
        ...
        FileRecord73 vo = (FileRecord73) frb;

        String sql = "insert into W_ACC_ST.W_ST_ERR_ENTRY_QRCODE(LINE_ID,STATION_ID,DEV_TYPE_ID,DEVICE_ID,SAM_LOGICAL_ID,"
                                             ...
                                             + "INTERVAL_BETWEEN_BUS_METRO,LAST_BUS_DEAL_DATETIME,BUSINESS_WATER_NO,MOBILE_NO "                                          
                                             + ") "
                                             + "values(?,?,?,?,?,"
                                             + "?,?,?,?,?,"
                                             + "?,?,?,?,?,"
                                             + "?,?,?,?,?,"
                                             + "?,W_ACC_ST.W_SEQ_W_ST_ERR_ENTRY_QRCODE.nextval,?,?,?,"
                                             + "?,?,?,?,?, "  
                                              + "?,?,?,? "  
                                              + ") ";
        ...
        Object[] values = {vo.getLineId(),vo.getStationId(),vo.getDevTypeId(),vo.getDeviceId(),vo.getSamLogicalId(),
                           ...
                           vo.getIntervalBetweenBusMetro_s(),vo.getLastBusDealDatetime_s(),vo.getBusinessWaterNo(),vo.getMobileNo()
                          };
        try {
