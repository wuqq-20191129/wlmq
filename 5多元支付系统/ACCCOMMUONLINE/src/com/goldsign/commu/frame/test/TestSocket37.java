 ...
 * @author goldsign
 */
public class TestSocket37 extends Thread{
...
//    public static final String ipAddress = "210.21.94.85";
    ...
    public static void main(String[] args) {
        TestSocket37 myThread;
        myThread = new TestSocket37();
        myThread.run();
    }
...
//                }else if(i==2){
                ...
                if(dataType==3){
                    ...
                    sb.append(","+getBcdString(data, 58, 7));//有效时间
                    sb.append(","+getLong(data, 65));//发售单程票总数量
                    sb.append(","+getLong(data, 69));//已取单程票数量
                    sb.append(","+getLong(data, 73));//发售单程票单价
                    sb.append(","+getBcdString(data,77,7));//取票时间
                    sb.append(","+getCharString(data, 84, 2));//响应码
                    sb.append(","+getCharString(data, 86, 2));//错误码
                }
                sb.append(","+in.read());
...
//                    socket.close();
    ...
    private byte[] data(QrCodeVo vo){
        byte[] b = new byte[52];
        System.arraycopy("37".getBytes(), 0, b, 0, 2);//消息类型
