 ...
 * 空中充值确认
 */
public class Message33 extends MessageValidate{
    
    private final static byte[] LOCK = new byte[0];
    private static Logger logger = Logger.getLogger(Message33.class.getName());
    private static final String tranType = "14";
    //序列标签
    private static final String SEQ_LABLE = FrameDBConstant.COM_OL_P + "seq_" + FrameDBConstant.TABLE_PREFIX + "ol_air_charge_cfm";
    ...
    private void processMessage() throws Exception {
        ...
        try {
//            logger.info("-- 解析空中充值确认消息33 --");
            ArrayList logList = new ArrayList();
            logList.add("空中充值确认33消息");
            ...
            airDao.insertAirChargeCfm(airChargeCfmVo, getOLDbHelper());
...
//            writeBackMsg(airChargeCfmVo);
            airDao.insertAirChargeCfmResp(airChargeCfmVo, getOLDbHelper());
            // 更新统计表w_ol_sts_acc
            if("00".equals(airChargeCfmVo.getReturnCode())){
                updateStsAcc(airChargeCfmVo);
            }
            writeBackMsg(airChargeCfmVo);
//            logger.info("--空中充值确认结束--");
            logger.info(new ThreadLocalUtil().ArrayToString(logList, true));
        } catch (Exception e) {
//            logger.error("处理33消息出现异常", e);
            // 出现异常返回响应的错误信息给终端
            dealException(airChargeCfmVo, e);
            logger.error(new ThreadLocalUtil().ArrayToString(ThreadLocalUtil.LOG_THREAD_LOCAL.get(), false) + "处理33消息出现异常", e);
    ...
    private void constructMessageVo(AirChargeCfmVo airChargeCfmVo) {
        ...
        AirDao airDao = new AirDao();
        try {
            result = airDao.checkAirCharge(airChargeCfmVo, getOLDbHelper());
        } catch (SQLException ex) {
            logger.error("查询空充申请响应数据库记录异常：", ex);
            ThreadLocalUtil.LOG_THREAD_LOCAL.get().add("查询空充申请响应数据库记录异常:"+ex.getMessage());
...
//        }
    ...
    private MessageQueue getMessageQueue(AirChargeCfmVo airChargeCfmVo) {
        byte[] msgVo = new ConstructMessage43().constructMessage(airChargeCfmVo);
    ...
    */
    private void updateStsAcc(AirChargeCfmVo airChargeCfmVo) {
        ...
        valueMap.put("air_charge_fee", airChargeCfmVo.getChargeFee()/100);
        valueMap.put("air_charge_num", 1);
