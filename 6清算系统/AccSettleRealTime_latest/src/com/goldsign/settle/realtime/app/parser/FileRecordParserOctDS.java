 ...
 * @author hejj
 */
public class FileRecordParserOctDS extends FileRecordParserBase{

    @Override
    public Object parse(String line, FileRecordAddVo lineAdd) throws RecordParseForFileException {
         FileRecordOctDS r = new FileRecordOctDS();
    ...
    public void handleMessage(MessageBase msg) {
     ...
     private FileRecord00DetailResult getInfoForRecord(FileRecordOctDS r, char[] b, int offset) throws Exception {
        FileRecord00DetailResult result = new FileRecord00DetailResult();


        int len = 32;
        r.setFileNameAudit(this.getCharString(b, offset, len));//oct结算的文件
        offset += len + 1;

        len = 10;
        r.setRecordNum(this.getIntFromStr(this.getCharString(b, offset, len)));//文件记录数     
        offset += len + 1;
        
         len = 12;
        r.setRecordFee(this.getBigDecimalFromStr(this.getCharString(b, offset, len)));//文件记录的金额   
        offset += len + 1;
