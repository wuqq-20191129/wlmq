package com.goldsign.ecpmcs.thread;
...
public class DbMonitorThread extends Thread{
    ...
    private void readLocalFiles() {
        String dir = BaseConstant.appWorkDir+ConfigUtil.getConfigValue(ConfigConstant.UploadTag, ConfigConstant.UploadLocalFileTag);
        File file = new File(dir);
        if(file.isDirectory()){
            String[] fileList = file.list();
            if(fileList.length>0){
                for(int i=0;i<fileList.length;i++){
                    if(fileList[i].indexOf(AppConstant.LOCAL_FILE_TO_FILE_SUFFIX)>-1){
                        File readFile = new File(dir+"/"+fileList[i]);
                        if(readFile.exists()){
                            try {
                                if(LocalFile.readLocalFile(readFile)){//成功读取文件后删除操作
                                    readFile.delete();
                                }else{//读取文件失败后备份操作
                                    readFile.renameTo(new File(dir+"/"+fileList[i].replaceAll(AppConstant.LOCAL_FILE_TO_FILE_SUFFIX,AppConstant.LOCAL_FILE_TO_FILE_SUFFIX_BACK)));
                                }
