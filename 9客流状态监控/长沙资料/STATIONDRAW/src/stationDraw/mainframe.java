...
class mainframe
  ...
  /**  当前显示的车站的线路代码，车站代码。  */
  private String station = "01", sta_line = "02";
...
//表示是否按下Ctrl键
  ...
  public mainframe() {
    ...
    addWindowListener(new WindowAdapter() {
      public void windowClosing(WindowEvent evt) {
        try {
          drawpane.zujian.closeConnection();
          System.exit(0);
        }
        catch (Exception e) {
          e.printStackTrace();
    ...
    try {
      openData();
      stationlist = drawpane.zujian.getStation();
    }
    catch (Exception e) {
    ...
    drawpane.addMouseListener(new MouseAdapter() {
      ...
      public void mouseReleased(MouseEvent e) {
        if ((toolpane.gettsource() == 7)||(toolpane.gettsource() == 10)) {
      ...
      public void mouseClicked(MouseEvent evt) {
        ...
        if (mainframe.pressCtrl) {
          if (toolpane.gettsource() == 1) {
            int type = ( (Point) CURRENTSELECT.get(0)).x; //设备的类型变量type
            //设备的序号变量i
            int i = ( (Point) CURRENTSELECT.get(0)).y;
            CtrlSELECT.add(new Point(type, i));
          }
        }
        else {
            CtrlSELECT.clear();
          ...
          if (CURRENTSELECT.size() == 1 && evt.getClickCount() == 2) {
            //设备的类型变量type
            int type = ( (Point) CURRENTSELECT.get(0)).x;
            //设备的序号变量i
            int i = ( (Point) CURRENTSELECT.get(0)).y;
           changeDev(type, i);
          }
  ...
  public void insert(int x, int y) {
    ...
    if (flag) {
      ...
      if (!f) {
        CURRENTSELECT.clear();
        CURRENTSELECT.add(new Point(type, num));
      }
    }
      ...
      switch (toolpane.gettsource()) {
        ...
        case 2:
          ...
          if (dialog.getFlag()) {
            ...
            int b = drawpane.zujian.addDevice(toolpane.gettsource(), x, y,
                                              machineID,
            ...
            if (b < 0) {
              ...
              CURRENTSELECT.add(new Point(2, b));
            }
          }
          break;
        ...
        case 3: //插入双向闸机
          ...
          if (dialog2.getFlag()) {
            ...
            if (b < 0) {
              ...
              CURRENTSELECT.add(new Point(3, b));
            }
          }
        ...
        case 8: //插入文本
          ...
          if (textDialog.getflag()) {
            ...
            String type = textDialog.getType();
            if (text.length() > 0) {
              int b = drawpane.text.addText(text, fontsize, type, x, y);
              CURRENTSELECT.add(new Point(8, b));
            }
          }
          break;
        ...
        case 9: //插入图象
          ...
          if (receive == JFileChooser.APPROVE_OPTION) {
            if (open.getfile() != null) {
              ...
              if (imgurl.length() < 200) {
                ...
                if (b >= 0) {
                  CURRENTSELECT.add(new Point(9, b));
                }
              }
              else {
          ...
          //默认的情况下，当标志为4,5,6时,插入BOM,TVM,TCM
        default:
          if (((toolpane.gettsource() <= 6 && toolpane.gettsource() >= 4))||(toolpane.gettsource() >= 90)) {
            ...
            if (nextDialog.getFlag()) {
              ...
              int b =
                  drawpane.zujian.addDevice(toolpane.gettsource(), x, y,
                                            machineID,
                                            machinename, "");
              if (b < 0) {
                JOptionPane.showMessageDialog(this, "增加设备失败或设备的ID重复！");
                ...
                CURRENTSELECT.add(new Point(toolpane.gettsource(), b));
              }
            }
          }
      }
  ...
  /**
   * 该方法用来调用改变设备名称的方法。
   * @param type int 设备的类型
   * @param i int 设备的序号
   */
  public void changeDev(int type, int i) {
    drawpane.zujian.changename(this, type, i);
  }
  ...
  public void mouseDragged(MouseEvent evt) {
    ...
    if (toolpane.gettsource() == 1) {
      ...
      if (CURRENTSELECT.size() > 0) {
        for (int i = 0; i < CURRENTSELECT.size(); i++) {
          ...
          switch (type) {
            case 7: //移动线条
             drawpane.zujian.lineAr.moveXiant(num, movex, movey);
              break;
           case 8: //移动文本
            ...
            default: //当标志为2,3,4,5,6,移动对应的设备
              if (NAMEFLAG == 0) {
                drawpane.zujian.move(type, num, movex, movey);
              }
              //只移动设备的名称
              else {
                drawpane.zujian.moveName(type, num, movex, movey);
              }
          }
        }
        drawpane.repaint();
    ...
    //gettsource()等于7工具面板选择画线的按钮，拖动鼠标就代表画线
    else if ((toolpane.gettsource() == 7)||(toolpane.gettsource() == 10)) {
      int x = evt.getX();
      int y = evt.getY();
      ...
      if (xian_p2.x < 0) {
        ...
        int num = drawpane.zujian.lineAr.add(xian_p1, xian_p2);
      ...
      //当xian_p2点的X大于零时，表示当前拖动鼠标是接着刚画的那条线，继续画。
        ...
        drawpane.zujian.lineAr.changeLastXiant(xian_p1, xian_p2);
        drawpane.repaint();
      }
    }
    g.dispose();
  ...
  public void remove() {
    ...
    for (int i = CURRENTSELECT.size() - 1; i >= 0; i--) {
      ...
      switch (type) {
        case 7:
          drawpane.zujian.lineAr.removeXiant(num);
          break;
        case 8:
        ...
        default:
          if (((type >= 2 && type <= 6))||(type >= 90)) {
            if (NAMEFLAG != 1) {
              ...
              drawpane.zujian.remove(type, num);
            }
          }
      }
    }
  ...
  public void saveFlush(String direction) {
    ...
    if (CtrlSELECT.size() > 1) {
      Device one = drawpane.zujian.findDev( ( (Point) CtrlSELECT.get(0)).x);
      type = ( (Point) CtrlSELECT.get(0)).x;
      num = ( (Point) CtrlSELECT.get(0)).y;
      switch (type) {
        case 7: {
          xx = drawpane.zujian.lineAr.getXiant(num).one.x;
          yy = drawpane.zujian.lineAr.getXiant(num).one.y;
          break;
        }
       case 8:{
      ...
      for (int i = 1; i < CtrlSELECT.size(); i++) {
        ...
        switch (type) {
          case 7:
            drawpane.zujian.lineAr.moveXiantFulsh(num, xx, yy, direction);
            break;
         ...
         case 8:
          ...
          default:
            if (((type >= 2 && type <= 6))||(type >= 90)) {
              if (NAMEFLAG == 0) { //加入判断:是否当前选中的是设备还是名称,再做移动操作
                drawpane.zujian.moveFush(type, num, direction, xx, yy);
              }
              else { //如果名称标志为1，那么表示选中了名称，那么
                drawpane.zujian.moveNameFush(type, num, direction, xx, yy);
              }
              break;
            }
