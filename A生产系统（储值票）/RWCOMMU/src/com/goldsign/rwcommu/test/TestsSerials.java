 ...
 * @author mh
 */
public class TestsSerials {

    private static RWSerialConnection serialConnection = null;
    public Runtime run = null;

    static {
        serialConnection = new RWSerialConnection("COM6");
...
//        esAnalyze();
    ...
    public static void commonInitializeDevice() throws SerialException, IOException {
        ...
        bt.put(bcdStringToByteArray("0000"));
        bt.put(bcdStringToByteArray("09"));
        bt.put(bcdStringToByteArray("120"));
        //byte[] bytes = new byte[]{(byte) 0x63, (byte) 0x00, (byte) 0x09, (byte) 0x01, (byte) 0x14};
        System.out.println("环境初始化1：");
        byte[] bytesRet = serialConnection.callSerial(command, bt.array());
    ...
    public static void esAnalyze() throws IOException, SerialException {
        ...
        String cLine = Converter.bytesToAsciiStringNoSpace(getByteArr(bytesRet, 108, 2));
        System.out.println("cLine:" + cLine);
        String cStationNo =  Converter.bytesToAsciiStringNoSpace(getByteArr(bytesRet, 110, 2));
        ...
        String dtDaliyActive = bytesToHexStringNoSpace(getByteArr(bytesRet, 128, 7));
    ...
    private static void initEvaluate() {
        ...
        try {
            ...
            bytes.put(testFlag);
            byte[] issueDate = CharUtil.bcdStrToLenByteArr("20180515", 4);
            bytes.put(issueDate);
            byte[] cardVersion = CharUtil.bcdStrToLenByteArr("01", 2);
            bytes.put(cardVersion);
            byte[] cardStartDate = CharUtil.bcdStrToLenByteArr("20180517", 4);
            bytes.put(cardStartDate);
            byte[] cardEndDate = CharUtil.bcdStrToLenByteArr("21190902", 4);
            bytes.put(cardEndDate);
            //-----逻辑日期
            byte[] logicDate = CharUtil.bcdStrToLenByteArr("00000000", 7);
            bytes.put(logicDate);
            byte[] logicDays = CharUtil.to2Byte(CharUtil.strToInt("000"));
            bytes.put(logicDays);
            //-----
            ...
            byte[] entryLineStation = CharUtil.bcdStrToLenByteArr("0102", 2);
            bytes.put(entryLineStation);
            byte[] exitLineStation = CharUtil.bcdStrToLenByteArr("0201", 2);
            ...
            if (ticketType[0] == 1) {
               ...
               /* CardKeyResult cardKeyResult = (CardKeyResult) keyms(cardNo, true).getObj();
                System.out.println("返回秘钥1：\n" + Converter.bytesToAsciiStringNoSpace(cardKeyResult.getMsg()));
                System.out.println("返回秘钥：\n" + Converter.bytesToHexString(cardKeyResult.getMsg()));
                CardKeyResult cardKeyResult = new CardKeyResult();
    ...
    private static void addKmsByByte(ByteBuffer bytes, byte[] bytess) {
        ...
        //bytes.put(CharUtil.hexStrToLenByteArr("                ",16));
    ...
    private static void addKmsByByte1(ByteBuffer bytes, String str) {
        bytes.put(CharUtil.hexStrToLenByteArr("FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF", 16));//MfCardTranKey
        bytes.put(CharUtil.hexStrToLenByteArr("FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF", 16));
        System.out.println("MF卡片传输密钥:" + "FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF");
    ...
    private static CallResult keyms(String cardNo, boolean isPk) throws Exception {
        ...
        String ipStr = "";
        String[] ipArr = "172.20.16.21".split("\\.");
        for (int i = 0; i < 4; i++) {
    ...
    private static void destroy1() {
        ...
        try {
            ...
            String reqNo = "req1233211";
            bytes.put(CharUtil.strToLenByteArr(reqNo, 10));
            byte[] ticketType = CharUtil.hexStrToLenByteArr("0200", 2);
            bytes.put(ticketType);
    ...
    private static CallResult recode() {
        ...
        try {
            ...
            bytes.put(ticketType);
            byte command = (byte) 0x33;
            byte[] bytesRet3 = serialConnection.callSerial(command, null);
            String cardNo = getByteCharString(bytesRet3, 13, 20);
            cardNo = cardNo.trim();
            //String  cardNo = "8300000300000003";
            byte[] logicNo = CharUtil.hexStrToLenByteArr(cardNo, 8);
            bytes.put(logicNo);
            byte deposite = CharUtil.strToByte("100");
            bytes.put(deposite);
            int balance = CharUtil.strToInt("100");
            bytes.put(CharUtil.to4Byte(balance));
            byte[] topValue = CharUtil.to2Byte(CharUtil.strToInt("3211"));
            bytes.put(topValue);
            byte active = CharUtil.strToByte("05");
            bytes.put(active);
            byte[] senderCode = CharUtil.bcdStrToLenByteArr("0991", 2);
            bytes.put(senderCode);
            ...
            bytes.put(testFlag);
            byte[] issueDate = CharUtil.bcdStrToLenByteArr("20180601", 4);
            bytes.put(issueDate);
            byte[] cardVersion = CharUtil.bcdStrToLenByteArr("01", 2);
            bytes.put(cardVersion);
            byte[] cardStartDate = CharUtil.bcdStrToLenByteArr("20180602", 4);
            bytes.put(cardStartDate);
            byte[] cardEndDate = CharUtil.bcdStrToLenByteArr("20300702", 4);
            bytes.put(cardEndDate);
            //---
            byte[] logicDate = CharUtil.bcdStrToLenByteArr("20180903", 7);
            bytes.put(logicDate);
            ...
            byte exitEntryMode = CharUtil.strToByte("01");
            bytes.put(exitEntryMode);
            byte[] entryLineStation = CharUtil.bcdStrToLenByteArr("0101", 2);;
            bytes.put(entryLineStation);
            byte[] exitLineStation = CharUtil.bcdStrToLenByteArr("0203", 2);
            bytes.put(exitLineStation);
            if (ticketType[0] == 1) {
                cardNo = cardNo.substring(8, 16);
                byte command1 = (byte) 0x33;
            ...
            byte command3 = (byte) 0x37;
            byte[] bytesRet = serialConnection.callSerial(command3, bytes.array());
            String retCode = CharUtil.byteToIntStr(bytesRet, 0, 2);
            String relCode = CharUtil.getByteString(bytesRet, 2, 1);
    ...
    private static CallResult inadvEvaluate() {
        ...
        try {
            ...
            String reqNo = "req1233211";
            bytes.put(CharUtil.strToLenByteArr(reqNo, 10));
            byte[] ticketType = CharUtil.hexStrToLenByteArr("0100", 2);
            bytes.put(ticketType);
            byte command5 = (byte) 0x33;
            byte[] bytesRet3 = serialConnection.callSerial(command5, null);
            String cardNo = getByteCharString(bytesRet3, 13, 20);
            cardNo = cardNo.trim();
            byte[] logicNo = CharUtil.hexStrToLenByteArr(cardNo, 8);
            bytes.put(logicNo);
            byte deposite = CharUtil.strToByte("0");
            bytes.put(deposite);
            int balance = CharUtil.strToInt("1500");
            bytes.put(CharUtil.to4Byte(balance));
            byte[] topValue = CharUtil.to2Byte(CharUtil.strToInt("3211"));
            bytes.put(topValue);
            //---
            ...
            bytes.put(active);
            byte[] logicDate = CharUtil.bcdStrToLenByteArr("20180905", 7);
            bytes.put(logicDate);
            byte[] logicDays = CharUtil.to2Byte(CharUtil.strToInt("365"));
            bytes.put(logicDays);
            //---
    ...
    private static CallResult signCard() {
        ...
        try {
            ...
            //加入密钥
            String cardNo = "8303221000000001";
            CardKeyResult cardKeyResult = (CardKeyResult) keyms(cardNo, true).getObj();
            byte[] bytess = cardKeyResult.getMsg();
            bytes.put(CharUtil.hexStrToLenByteArr(CharUtil.getByteCharString(bytess, 288, 32), 16));//AppMendKey02
            ...
            String employeeName = StringUtil.addEmptyAfterUTF("古力", 128);
