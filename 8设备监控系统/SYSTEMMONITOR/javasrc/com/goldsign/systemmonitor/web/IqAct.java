...
public class IqAct extends BaseAction {
    ...
    private void convertText(Vector statues) throws Exception {
        ...
        Vector statuesFlagsJob = dbUtil.getPubFlagsByType(FrameDBConstant.Flag_type_etl, pubFlags);
        IqVo vo;


        for (int i = 0; i < statues.size(); i++) {
            vo = (IqVo) statues.get(i);
            vo.setStatusText(dbUtil.getTextByCode(vo.getStatus(), statuesFlags));
            vo.setJobStatusText(dbUtil.getTextByCode(vo.getJobStatus(), statuesFlagsJob));


        }
    }

    public ActionMessage query(HttpServletRequest request, IqDao pmb, Vector statues, String command) throws Exception {
