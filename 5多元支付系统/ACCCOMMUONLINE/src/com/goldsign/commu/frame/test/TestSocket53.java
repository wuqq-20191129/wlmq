 ...
 * @author goldsign
 */
public class TestSocket53 extends Thread{
...
//    public static final String ipAddress = "172.20.19.14";
    ...
    public static void main(String[] args) {
        TestSocket53 myThread;
        myThread = new TestSocket53();
        myThread.run();
    }
    ...
    public void run(){
        ...
        try {
            ...
            for (int i=0; i<10; i++) {
                ...
                if(i==0){
                    Message53Vo vo = new Message53Vo();
                    vo.setTerminalNo("012103151");
                    vo.setSamLogicalId("83000003B0000026");
                    vo.setTransationSeq(1);
                    vo.setBranchesCode("0000000000000000");
                    vo.setPubMainCode("0991");
                    vo.setPubSubCode("0000");
                    vo.setCardType("0201");
                    vo.setTkLogicNo("8303211000000003");
                    vo.setTkPhyNo("8303211000000003");
                    vo.setIsTestTk("1");
                    vo.setOnlTranTimes(1L);
                    vo.setOfflTranTimes(1L);
                    vo.setBussType("18");
                    vo.setValueType("1");
                    vo.setChargeFee(20000L);
                    vo.setBalance(44900L);
                    vo.setLastTranTermNo("83000003B0000026");
                    vo.setLastDealTime("20180625131338");
                    vo.setOperatorId("000000");
...
//                }else if(i==2){
                ...
                if(dataType==3){
                    ...
                    sb.append(","+getLong(data, 9));//系统参照号
                    sb.append(","+getCharString(data, 13, 9));//BOM代码
                    sb.append(","+getCharString(data, 22, 16));//sam卡号
                    sb.append(","+getCharString(data, 38, 16));//mac
                    sb.append(","+getCharString(data, 54, 2));//响应码
                    sb.append(","+getCharString(data, 56, 2));//错误描述
                }
                sb.append(","+in.read());
...
//                    socket.close();
    ...
    private byte[] data(Message53Vo vo){
        byte[] b = new byte[153];
        System.arraycopy("53".getBytes(), 0, b, 0, 2);//消息类型
        ...
        System.arraycopy(longStringToByteArray(vo.getTransationSeq(),4), 0, b, 34, 4);//
        System.arraycopy(vo.getBranchesCode().getBytes(), 0, b, 38, 16);//
        System.arraycopy(vo.getPubMainCode().getBytes(), 0, b, 54, 4);//
        System.arraycopy(vo.getPubSubCode().getBytes(), 0, b, 58, 4);//
        System.arraycopy(bcdStringToByteArray(vo.getCardType()), 0, b, 62, 2);//
        System.arraycopy(CharUtil.addCharsAfter(vo.getTkLogicNo(),20," ").getBytes(), 0, b, 64,20);//
        System.arraycopy(CharUtil.addCharsAfter(vo.getTkPhyNo(),20," ").getBytes(), 0, b, 84, 20);//
        System.arraycopy(vo.getIsTestTk().getBytes(), 0, b, 104, 1);//
        System.arraycopy(longStringToByteArray(vo.getOnlTranTimes(),4), 0, b, 105, 4);//
        System.arraycopy(longStringToByteArray(vo.getOfflTranTimes(),4), 0, b, 109, 4);//
        System.arraycopy(vo.getBussType().getBytes(), 0, b, 113, 2);//
        System.arraycopy(vo.getValueType().getBytes(), 0, b, 115, 1);//
        System.arraycopy(longStringToByteArray(vo.getChargeFee(),4), 0, b, 116, 4);//
        System.arraycopy(longStringToByteArray(vo.getBalance(),4), 0, b, 120, 4);//
        System.arraycopy(CharUtil.addCharsAfter(vo.getLastTranTermNo(),16," ").getBytes(), 0, b, 124, 16);//
        System.arraycopy(bcdStringToByteArray(vo.getLastDealTime()), 0, b, 140, 7);//
        System.arraycopy(vo.getOperatorId().getBytes(), 0, b, 147, 6);//
