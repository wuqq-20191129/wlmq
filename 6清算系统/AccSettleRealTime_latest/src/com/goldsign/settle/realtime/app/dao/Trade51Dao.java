 ...
 * @author hejj
 */
public class Trade51Dao extends TradeBaseDao{
     private static Logger logger = Logger.getLogger(Trade51Dao.class.getName());
    ...
    public int insertError(FileRecordBase frb,String errCode)  {
        ...
        FileRecord51 vo = (FileRecord51) frb;

        String sql = "insert into W_ACC_ST.W_ST_ERR_SALE(LINE_ID,STATION_ID,DEV_TYPE_ID,DEVICE_ID,CARD_MAIN_ID,"
                                             ...
                                             + "mobile_no,RECORD_VER,sale_fee "                                            
                                             + ") "
                                             + "values(?,?,?,?,?,"
                                             + "?,?,?,?,?,"
                                             + "?,?,?,?,?,"
                                             + "?,?,?,?,?,"
                                             + "?,?,?,?,W_ACC_ST.W_SEQ_W_ST_ERR_SALE.nextval,?,"
                                             + "?,?,?"                                             
                                             + ") ";
        ...
        Object[] values = {vo.getLineId(),vo.getStationId(),vo.getDevTypeId(),vo.getDeviceId(),vo.getCardMainId(),
                           ...
                           vo.getMobileNo(),vo.getRecordVer(),vo.getSaleFee_s()
                          };
        try {
