package com.goldsign.sammcs.thread;
...
public class DbMonitorThread extends Thread{
    ...
    private void checkDBConnect(){
        ...
        //检测数据库连接
        AppConstant.COMMU_STATUS = AppConstant.dbcpHelper.checkDBConnect();
    ...
    private void readMakeCardFiles() {
        String dir = BaseConstant.appWorkDir+ConfigUtil.getConfigValue(ConfigConstant.BackupTag, ConfigConstant.BackupMakeCardTag);
            ...
            if(fileList.length>0){
                for(int i=0;i<fileList.length;i++){
                    if(fileList[i].indexOf(AppConstant.MAKE_CARD_TO_FILE_SUFFIX)>-1){
                        ...
                        if(readFile.exists()){
                            try {
                                if(makeCardService.readLocalFile(readFile,BaseConstant.user.getAccount())){//成功读取文件后删除操作
                                    readFile.delete();
                                }else{//读取文件失败后备份操作
