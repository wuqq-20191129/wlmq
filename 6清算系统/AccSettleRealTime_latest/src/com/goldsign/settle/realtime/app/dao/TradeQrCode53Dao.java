 ...
 * @author hejj
 */
public class TradeQrCode53Dao extends TradeBaseDao{
    private static Logger logger = Logger.getLogger(TradeQrCode53Dao.class.getName());
    ...
    public int insertError(FileRecordBase frb,String errCode)  {
        ...
        FileRecordQrCode53 vo = (FileRecordQrCode53) frb;

        String sql = "insert into W_ACC_ST.W_ST_QT_ERR_ENTRY(LINE_ID,STATION_ID,DEV_TYPE_ID,DEVICE_ID,SAM_LOGICAL_ID,"
                                             ...
                                             + "INTERVAL_BETWEEN_BUS_METRO,LAST_BUS_DEAL_DATETIME,BUSINESS_WATER_NO,MOBILE_NO,ISSUE_QRCODE_PLATFORM_FLAG "                                          
                                             + ") "
                                             + "values(?,?,?,?,?,"
                                             + "?,?,?,?,?,"
                                             + "?,?,?,?,?,"
                                             + "?,?,?,?,?,"
                                             + "?,W_ACC_ST.W_SEQ_W_ST_QT_ERR_ENTRY.nextval,?,?,?,"
                                             + "?,?,?,?,?, "  
                                              + "?,?,?,?,? "  
                                              + ") ";
        ...
        Object[] values = {vo.getLineId(),vo.getStationId(),vo.getDevTypeId(),vo.getDeviceId(),vo.getSamLogicalId(),
                           ...
                           vo.getIntervalBetweenBusMetro_s(),vo.getLastBusDealDatetime_s(),vo.getBusinessWaterNo(),vo.getMobileNo(),vo.getIssueQrcodePlatformFlag()
                          };
        try {
