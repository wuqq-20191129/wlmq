...
// function openWindowDialog(formName,valueCtrName,jspName,moduleID,width,height){
  function openWindowDialog(formName,valueCtrName,controllerName,width,height){ //20170912 mqf
//	var path=jspName+"?ModuleID="+moduleID;
        var path=controllerName;
//	alert(path);
...
//       document.getElementById(valueCtrName).value = rt;
...
function openWindowDialogForParamDis(formName,valueCtrName,paramCtrNames,isChecks,msgs,paramCtrNames1,msgs1,controllerName,moduleID,width,height){
	var path=controllerName+"?ModuleID="+moduleID;
    var param ='';
    if(document.getElementById(paramCtrNames1[0]).value != ''){
    	for(var j=0;j<paramCtrNames1.length;j++){
    		if(document.getElementById(paramCtrNames1[j]).value == ''){
//    		  alert('出库原因不能为空');
                  //20151209 modify by mqf
                  alert(msgs1[j]+'不能为空');
	    	  return;
    		}
    	}
	...
	for(var i=0;i<paramCtrNames.length;i++){
	    if(document.getElementById(paramCtrNames[i]).value ==''&&isChecks[i]){
	    	alert('出库原因不能为空');
	    	return;
	    	}
		param +='&'+document.getElementById(paramCtrNames[i]).name+'='+document.getElementById(paramCtrNames[i]).value;
		}
	path +=param;
        path +='&operType=lineContent';
//	alert(path+param);
	var rt=window.showModalDialog(path,'','dialogWidth:'+width+'px;dialogHeight:'+height+'px;center:yes;resizable:no;status:no;scroll:yes');
//	alert(rt);
	if(rt!=null && rt!='undefined')
