...
import com.goldsign.acc.frame.vo.ImportLogVo;
...
/**
 *
 * @author liudz 线路交易分配比例
 */
@Controller
public class DealAssignLineController extends PrmBaseController {

    Logger logger = Logger.getLogger(DealAssignLineController.class);
    @Autowired
    private DealAssignLineMapper dealAssignLineMapper;
    @Autowired
    private OpLogImportMapper opLogImportMapper;
    @Autowired
    private PubFlagMapper pubFlagMapper;

    @RequestMapping("/DealAssignLine")
    public ModelAndView service(HttpServletRequest request, HttpServletResponse response) {

        ModelAndView mv = new ModelAndView("/jsp/prminfo/deal_assign_line.jsp");
        ...
        try {
            if (command != null) {
                ...
                {
                    opResult = this.modify(request, this.dealAssignLineMapper, this.prmVersionMapper, this.operationLogMapper);
                }
                if (command.equals(CommandConstant.COMMAND_DELETE))//删除操作
                {
                    opResult = this.delete(request, this.dealAssignLineMapper, this.prmVersionMapper, this.operationLogMapper);
                }
                if (command.equals(CommandConstant.COMMAND_ADD))//增加操作
                {
                    opResult = this.add(request, this.dealAssignLineMapper, this.prmVersionMapper, this.operationLogMapper);
                }

                if (command.equals(CommandConstant.COMMAND_QUERY))//查询操作
                {
                    opResult = this.query(request, this.dealAssignLineMapper, this.operationLogMapper);

                }
                if (command.equals(CommandConstant.COMMAND_SUBMIT))//提交操作
                {
                    opResult = this.submit(request, this.dealAssignLineMapper, this.prmVersionMapper, this.operationLogMapper);
                }
                if (command.equals(CommandConstant.COMMAND_CLONE))//克隆操作
                {
                    opResult = this.clone(request, this.dealAssignLineMapper, this.prmVersionMapper, this.operationLogMapper);
                }
                if (command.equals("importLine"))//打开导入页面操作
                {
                    mv = new ModelAndView("/jsp/prminfo/importDealLinePara.jsp");
                    return mv;
                }
            }
            ...
            if (command != null) {
                if (command.equals(CommandConstant.COMMAND_ADD) || command.equals(CommandConstant.COMMAND_DELETE) || command.equals(CommandConstant.COMMAND_MODIFY)
                    ...
                    this.queryForOp(request, this.dealAssignLineMapper, this.operationLogMapper, opResult);
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
        ...
        //初始化下拉框
        String[] attrNames = {LINES, STATIONS, LINE_STATIONS, VERSION};

        this.setPageOptions(attrNames, mv, request, response);//设置页面线路、车站等选项值、版本
        this.getResultSetText((List<DealAssignLine>) opResult.getReturnResultSet(), mv);
    ...
    private void getResultSetText(List<DealAssignLine> resultSet, ModelAndView mv) {
        //线路名
        ...
        for (DealAssignLine dl : resultSet) {
            if (dl.getB_line_id() != null && !dl.getB_line_id().isEmpty()) {
                dl.setB_line_id_name(DBUtil.getTextByCode(dl.getB_line_id(), lines));
            }

            if (dl.getE_line_id() != null && !dl.getE_line_id().isEmpty()) {
                dl.setE_line_id_name(DBUtil.getTextByCode(dl.getE_line_id(), lines));
            }
            if (dl.getLine_id_dispart() != null && !dl.getLine_id_dispart().isEmpty()) {
                dl.setLine_id_dispart_name(DBUtil.getTextByCode(dl.getLine_id_dispart(), lines));
            }
            //分账比例为小于零的小数时加上前面的0,为1时加上后面的0
            if (".".equals(dl.getIn_percent().substring(0, 1))) {
                dl.setIn_percent("0" + dl.getIn_percent());
                if (dl.getIn_percent().length() < 8) {
                    DecimalFormat df = new DecimalFormat("0.000000");
//                    System.out.println(df.format(0.34)); 
                    double num1 ;  
                    num1 = Double.valueOf(dl.getIn_percent().toString());
                    dl.setIn_percent(df.format(num1));
                }
            } else if ("1".equals(dl.getIn_percent().substring(0, 1))) {
                dl.setIn_percent(dl.getIn_percent().substring(0, 1) + ".000000");
            }else if ("0".equals(dl.getIn_percent().substring(0, 1))&&dl.getIn_percent().length()==1) {
                dl.setIn_percent(dl.getIn_percent().substring(0, 1) + ".000000");
            }

            if (stations != null && !stations.isEmpty()) {
                if (dl.getB_station_id() != null && !dl.getB_station_id().isEmpty()) {
                    dl.setB_station_id_name(DBUtil.getTextByCode(dl.getB_station_id(), dl.getB_line_id(), stations));
                }
                if (dl.getE_station_id() != null && !dl.getE_station_id().isEmpty()) {
                    dl.setE_station_id_name(DBUtil.getTextByCode(dl.getE_station_id(), dl.getE_line_id(), stations));
                }
            }
        }
    }

    private DealAssignLine getQueryConditionForOp(HttpServletRequest request) {
        DealAssignLine qCon = new DealAssignLine();
        ...
        if (FormUtil.isAddOrModifyMode(request)) {//操作处于添加或修改模式
            ...
            qCon.setLine_id_dispart(FormUtil.getParameter(request, "d_line_id_dispart"));
            ...
            if (!vQueryControlDefaultValues.isEmpty()) {
                ...
                qCon.setLine_id_dispart(FormUtil.getQueryControlDefaultValue(vQueryControlDefaultValues, "q_line_id_dispart"));
            }
        }
    ...
    //获取查询参数
    private DealAssignLine getQueryCondition(HttpServletRequest request) {
        ...
        String q_line_id_dispart = FormUtil.getParameter(request, "q_line_id_dispart");
        dealAssignLine.setB_line_id(q_b_line_id);
        dealAssignLine.setB_station_id(q_b_station_id);
        dealAssignLine.setE_line_id(q_e_line_id);
        dealAssignLine.setE_station_id(q_e_station_id);
        dealAssignLine.setLine_id_dispart(q_line_id_dispart);

//        //字段回显
//        request.setAttribute("q_line_id", q_line_id);
//        request.setAttribute("q_contc_id", q_contc_id);
        this.getBaseParameters(request, dealAssignLine);
        return dealAssignLine;
    }

    public OperationResult query(HttpServletRequest request, DealAssignLineMapper dealAssignLineMapper, OperationLogMapper opLogMapper) throws Exception {
        OperationResult or = new OperationResult();
        LogUtil logUtil = new LogUtil();
        DealAssignLine queryCondition;
        List<DealAssignLine> resultSet;
        try {
            queryCondition = this.getQueryCondition(request);
            resultSet = dealAssignLineMapper.getDealAssignLine(queryCondition);
     ...
     * 带查询条件查询线路记录
     */
    public OperationResult queryForOp(HttpServletRequest request, DealAssignLineMapper dealAssignLineMapper, OperationLogMapper opLogMapper, OperationResult opResult) throws Exception {
        LogUtil logUtil = new LogUtil();
        DealAssignLine dealAssignLine;
        List<DealAssignLine> resultSet;

        try {
            dealAssignLine = this.getQueryConditionForOp(request);
            if (dealAssignLine == null) {
                return null;
            }
            resultSet = dealAssignLineMapper.getDealAssignLine(dealAssignLine);
    ...
    public OperationResult modify(HttpServletRequest request, DealAssignLineMapper dealAssignLineMapper, PrmVersionMapper pvMapper, OperationLogMapper opLogMapper) throws Exception {
        OperationResult rmsg = new OperationResult();
        DealAssignLine dealAssignLine = this.getReqAttribute(request);
        LogUtil logUtil = new LogUtil();
        int n = 0;

        String preMsg = "交易分配比例：" + "主键：" + ":";
        try {
            n = this.modifyByTrans(request, dealAssignLineMapper, pvMapper, dealAssignLine);
        } catch (Exception e) {
    ...
    private boolean existRecord(DealAssignLine po, DealAssignLineMapper dealAssignLineMapper, OperationLogMapper opLogMapper) {
        List<DealAssignLine> list = dealAssignLineMapper.getDealAssignLineById(po);
        if (list.isEmpty()) {
            return false;
        }
        return true;
    ...
    private int addByTrans(HttpServletRequest request, DealAssignLineMapper dealAssignLineMapper, PrmVersionMapper pvMapper, DealAssignLine dealAssignLine) throws Exception {

        TransactionStatus status = null;
        int n = 0;
        try {
            status = txMgr.getTransaction(this.def);
            n = dealAssignLineMapper.addDealAssignLine(dealAssignLine);
            pvMapper.modifyPrmVersionForDraft(dealAssignLine);
    ...
    private void getVersionNoForSubmit(DealAssignLine po, String versionNoMax) {
        String max;
        int n;
    ...
    public int submitByTrans(HttpServletRequest request, DealAssignLineMapper dealAssignLineMapper, PrmVersionMapper pvMapper, DealAssignLine dealAssignLine, PrmVersion prmVersion) throws Exception {
        ...
        try {
            ...
            //旧的未来或当前参数数据做删除标志
            dealAssignLineMapper.submitToOldFlag(dealAssignLine);
            //生成新的未来或当前参数的版本号
            versionNoMax = pvMapper.selectPrmVersionForSubmit(dealAssignLine);
            this.getVersionNoForSubmit(dealAssignLine, versionNoMax);

            //添加新的未来或当前参数的数据记录
            n = dealAssignLineMapper.submitFromDraftToCurOrFur(dealAssignLine);
            // 重新生成参数表中的未来或当前参数记录
            pvMapper.modifyPrmVersionForSubmit(dealAssignLine);
            pvMapper.addPrmVersion(dealAssignLine);
    ...
    private int cloneByTrans(HttpServletRequest request, DealAssignLineMapper dealAssignLineMapper, PrmVersionMapper pvMapper, DealAssignLine dealAssignLine, PrmVersion prmVersion) throws Exception {
        TransactionStatus status = null;
        int n = 0;
        try {
            status = txMgr.getTransaction(this.def);
            ...
            //删除草稿版本
            dealAssignLineMapper.deleteDealAssignLineForClone(dealAssignLine);
            //未来或当前参数克隆成草稿版本
            n = dealAssignLineMapper.cloneFromCurOrFurToDraft(dealAssignLine);
            //更新参数版本索引信息
            pvMapper.modifyPrmVersionForDraft(dealAssignLine);
    ...
    public OperationResult submit(HttpServletRequest request, DealAssignLineMapper dealAssignLineMapper, PrmVersionMapper pvMapper, OperationLogMapper opLogMapper) throws Exception {
        OperationResult rmsg = new OperationResult();
        DealAssignLine po = this.getReqAttributeForSubmit(request);
        DealAssignLine prmVersion = null;
        LogUtil logUtil = new LogUtil();
        int n = 0;

        try {
            n = this.submitByTrans(request, dealAssignLineMapper, pvMapper, po, prmVersion);
        } catch (Exception e) {
    ...
    public OperationResult clone(HttpServletRequest request, DealAssignLineMapper dealAssignLineMapper, PrmVersionMapper pvMapper, OperationLogMapper opLogMapper) throws Exception {
        OperationResult rmsg = new OperationResult();
        DealAssignLine po = this.getReqAttributeForClone(request);
        PrmVersion prmVersion = null;
        LogUtil logUtil = new LogUtil();
        int n = 0;

        try {
            n = this.cloneByTrans(request, dealAssignLineMapper, pvMapper, po, prmVersion);
        } catch (Exception e) {
    ...
    private int modifyByTrans(HttpServletRequest request, DealAssignLineMapper dealAssignLineMapper, PrmVersionMapper pvMapper, DealAssignLine dealAssignLine) throws Exception {

        TransactionStatus status = null;
        int n = 0;
        try {
            status = txMgr.getTransaction(this.def);
            n = dealAssignLineMapper.modifyDealAssignLine(dealAssignLine);
            pvMapper.modifyPrmVersionForDraft(dealAssignLine);
    ...
    private int deleteByTrans(HttpServletRequest request, DealAssignLineMapper dealAssignLineMapper, PrmVersionMapper pvMapper, Vector<DealAssignLine> pos, DealAssignLine prmVo) throws Exception {

        TransactionStatus status = null;
        int n = 0;
        try {
            status = txMgr.getTransaction(this.def);
            for (DealAssignLine po : pos) {
                n += dealAssignLineMapper.deleteDealAssignLine(po);
            }
    ...
    public OperationResult add(HttpServletRequest request, DealAssignLineMapper dealAssignLineMapper, PrmVersionMapper pvMapper, OperationLogMapper opLogMapper) throws Exception {
        OperationResult rmsg = new OperationResult();
        DealAssignLine po = this.getReqAttribute(request);
        LogUtil logUtil = new LogUtil();
        int n = 0;
        String preMsg = "交易分配比例：" + "主键：" + ":";
        try {
            if (this.existRecord(po, dealAssignLineMapper, opLogMapper)) {
                rmsg.addMessage("此交易分配比例记录已存在！");
                return rmsg;
            }

            n = this.addByTrans(request, dealAssignLineMapper, pvMapper, po);
...
//        }
    ...
    public OperationResult delete(HttpServletRequest request, DealAssignLineMapper dealAssignLineMapper, PrmVersionMapper pvMapper, OperationLogMapper opLogMapper) throws Exception {
        OperationResult rmsg = new OperationResult();
        Vector<DealAssignLine> pos = this.getReqAttributeForDelete(request);
        DealAssignLine prmVo = new DealAssignLine();
        this.getBaseParameters(request, prmVo);
        LogUtil logUtil = new LogUtil();
        int n = 0;

        try {
            n = this.deleteByTrans(request, dealAssignLineMapper, pvMapper, pos, prmVo);
        } catch (Exception e) {
    ...
    public DealAssignLine getReqAttribute(HttpServletRequest request) {
        DealAssignLine dealAssignLine = new DealAssignLine();

        dealAssignLine.setB_line_id(FormUtil.getParameter(request, "d_b_line_id"));
        dealAssignLine.setB_station_id(FormUtil.getParameter(request, "d_b_station_id"));
        dealAssignLine.setE_line_id(FormUtil.getParameter(request, "d_e_line_id"));
        dealAssignLine.setE_station_id(FormUtil.getParameter(request, "d_e_station_id"));
        dealAssignLine.setLine_id_dispart(FormUtil.getParameter(request, "d_line_id_dispart"));
        dealAssignLine.setIn_percent(FormUtil.getParameter(request, "d_in_percent"));
    ...
    public DealAssignLine getReqAttributeForSubmit(HttpServletRequest request) {
        DealAssignLine dealAssignLine = new DealAssignLine();

        getBaseParameters(request, dealAssignLine);
        getBaseParametersForSubmit(request, dealAssignLine);

        return dealAssignLine;

    }

    public DealAssignLine getReqAttributeForClone(HttpServletRequest request) {
        DealAssignLine dealAssignLine = new DealAssignLine();
    ...
    private Vector<DealAssignLine> getDeleteIDs(String selectedIds) {
        StringTokenizer st = new StringTokenizer(selectedIds, ";");
        String strIds = null;
        Vector<DealAssignLine> sds = new Vector();
        DealAssignLine sd;
        while (st.hasMoreTokens()) {
            strIds = st.nextToken();
            sd = this.getDealAssignLine(strIds, "#");
            sds.add(sd);
        }
        return sds;
    ...
    private Vector<DealAssignLine> getReqAttributeForDelete(HttpServletRequest request) {
        String selectIds = request.getParameter("allSelectedIDs");
        Vector<DealAssignLine> selectedItems = this.getDeleteIDs(selectIds);

        return selectedItems;
    }

    private String getRecordFlagOld(DealAssignLine dealAssignLine) {
        String recordFlagSubmit = dealAssignLine.getRecord_flag_submit();
    ...
    private String getVersionValidDate(DealAssignLine dealAssignLine) {
        String verDateBegin = dealAssignLine.getBegin_time_submit();
    ...
    @RequestMapping("/import_DealLine")
    public ModelAndView import_DealLine(HttpServletRequest request, HttpServletResponse response) {
        ModelAndView mv = new ModelAndView("/jsp/prminfo/importDealLinePara.jsp");
        OperationResult opResult = new OperationResult();
...
//         this.SaveOperationResult(mv, opResult);
    ...
    private OperationResult importFile(HttpServletRequest request, PrmVersionMapper pvMapper, OperationLogMapper opLogMapper, ModelAndView mv) throws Exception {
        OpLogImport uploadForm = new OpLogImport();
        LogUtil logUtil = new LogUtil();
        int n = 0;
...
//                    + "条记录");
    ...
    private int importDealAssginLineBySql(HttpServletRequest request, InputStream stream, String terminator) throws Exception {
        ...
        Vector<String> lines;
        linestations = dealAssignLineMapper.getLines();
        lines = dealAssignLineMapper.getStations();
        ...
        ImportResult r4 = this.imporFileForDealAssignBySql(request, stream, ImportConstant.TABLE_NAME_DEAL_LINE,
                ...
                ImportConstant.FIELD_NAME_DEAL_LINE, ImportConstant.FIELD_TYPE_DEAL_LINE, linestations, lines);
        n = r4.getRecordNum();
        return n;
    }
    ...
    public ImportResult imporFileForDealAssignBySql(HttpServletRequest request,
        ...
        DealAssignLine result = new DealAssignLine();
...
//            n = dealAssignLineMapper.deleteVersionRecordWithTran(tableName, impResult.getVersonNo());
...
//            PubUtil.finalProcessForTran(dbHelper, logger);
     ...
     * @throws Exception
     */
    protected DealAssignLine insertRecordsByIndexWithTran(ImportResult impResult,
        ...
        int n = 0;
        List<DealAssignLine> listTemps = new ArrayList<DealAssignLine>();
        for (int a = 0; a < records.size(); a++) {
            try {

                String[] a1 = (String[]) records.get(a);
                DealAssignLine dl = new DealAssignLine();
                ...
                n += dealAssignLineMapper.addDealAssignLine(dl);
                listTemps.add(dl);

            } catch (Exception e) {
                DealAssignLine d2 = new DealAssignLine();
                for (int j = 0; j < listTemps.size(); j++) {
                    d2 = listTemps.get(j);
                    try {
                        dealAssignLineMapper.deleteDealAssignLine(d2);
                    } catch (Exception e1) {
                        e1.printStackTrace();
                    }
                }
...
//            }
    ...
    private boolean checkFieldsByBusinessForConcts(String cont, Vector concts,
        ...
        if (!concts.contains(cont)) {
            throw new Exception(title + "文件" + "的第" + i + "行线路不存在");
        }
        return true;
    }
    ...
    @RequestMapping("/DealAssignLineExportAll")
    public void exportExcel(HttpServletRequest request, HttpServletResponse response) throws Exception {
    	
        List results = this.getBufferElementsForCurClass(request, "com.goldsign.acc.app.prminfo.entity.DealAssignLine");
    ...
    private List<Map<String, String>> entityToMap(List results) {
        ...
        for(int n=0;n<row ;n++){
            DealAssignLine vo = (DealAssignLine)results.get(n);
            ...
            map.put("line_id_dispart_name", vo.getLine_id_dispart_name());
