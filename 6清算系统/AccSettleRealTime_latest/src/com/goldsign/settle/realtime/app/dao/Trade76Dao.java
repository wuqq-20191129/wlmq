 ...
 * @author hejj
 */
public class Trade76Dao extends TradeBaseDao{
      private static Logger logger = Logger.getLogger(Trade76Dao.class.getName());
    ...
    public int insertError(FileRecordTacBase frb) throws Exception {
     ...
     public int insertError(FileRecordBase frb,String errCode)  {
        ...
        FileRecord76 vo = (FileRecord76) frb;

        String sql = "insert into W_ACC_ST.W_ST_ERR_UPDATE_QRCODE(LINE_ID,STATION_ID,DEV_TYPE_ID,DEVICE_ID,SAM_LOGICAL_ID,"
                                             ...
                                             + "BUSINESS_WATER_NO,BUSINESS_WATER_NO_REL "                                          
                                             + ") "
                                             + "values(?,?,?,?,?,"
                                             + "?,?,?,?,?,"
                                             + "?,?,?,?,?,"
                                             ...
                                             + "?,?,?,?,?,"
                                             + "?,?,?,?,?," 
                                             + "W_ACC_ST.W_SEQ_W_ST_ERR_UPDATE_QRCODE.nextval,?,?,?,?,"
                                             + "?,?"               
                                              + ") ";
        ...
        Object[] values = {vo.getLineId(),vo.getStationId(),vo.getDevTypeId(),vo.getDeviceId(),vo.getSamLogicalId(),
                           ...
                           vo.getBusinessWaterNo(),vo.getBusinessWaterNoRel()
                          };
        try {
