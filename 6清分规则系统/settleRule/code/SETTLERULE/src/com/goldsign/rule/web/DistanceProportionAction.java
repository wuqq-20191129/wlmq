...
public class DistanceProportionAction extends BaseAction {
   ...
   throws Exception {
        ...
        DistanceProportionService bo = new DistanceProportionService();
   ...
   /**
    ...
    private ActionMessage query(HttpServletRequest request, DistanceProportionService bo, Vector results) throws Exception {
        ...
        try{
            map = bo.queryStore(vo);
            results.addAll((Vector)map.getUpdateOb());
        ...
        if(map.getUpdateNum() == 1){
            am = new ActionMessage("queryMessage",FrameUtil.GbkToIso("成功查询"+results.size() +"条记录"));
        }else{
            am = new ActionMessage("queryMessage",FrameUtil.GbkToIso("成功失败："+map.getRetMsg()));
        }
        return am;
    }
