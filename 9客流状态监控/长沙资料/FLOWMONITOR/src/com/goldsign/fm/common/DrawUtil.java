...
public class DrawUtil {
    ...
    public static Color COLOR_IN = new Color(128, 255, 128); //进站颜色
    public static Color COLOR_OUT = new Color(255, 128, 128); //出站颜色
    //图片属性
    ...
    private final static Font FONT_PLOT_PIE_LABEL = new Font("黑体", Font.CENTER_BASELINE, 14); //餠图的字体
    ...
    JFreeChart getMultiPieForCard(Map<String, Vector> traffics, Vector dataSub) {
        Vector iSubVector = new Vector();//进子类型
        iSubVector = traffics.get(AppConstant.CARD_IN_SUB);
        Vector oSubVector = new Vector();//出子类型
        oSubVector = traffics.get(AppConstant.CARD_OUT_SUB);
        DefaultCategoryDataset dataset = new DefaultCategoryDataset();
        
        //进子类型
        for(int i=0; i < iSubVector.size(); i++){
            ViewVo vo = (ViewVo) iSubVector.get(i);
            dataset.setValue(Double.valueOf(vo.getTraffic_in()), vo.getSubCardName(), "进站子类型客流");
        }
        //出子类型
        for(int i=0; i < oSubVector.size(); i++){
            ViewVo vo = (ViewVo) oSubVector.get(i);
            dataset.setValue(Double.valueOf(vo.getTraffic_out()), vo.getSubCardName(), "出站子类型客流");
        }
        ...
        //设置xml配置文件票卡类型颜色
        setSectionColorForCard(plot, dataSub);
        //设置饼图正图属性
        setPiePlotCard(plot);
...
//        chart.addSubtitle(new TextTitle("setCircular(false);", new Font("Dialog", Font.PLAIN, 12)));//设置子标题
    ...
    public void setSectionColorForCard(Plot ploter, Vector drawResultV) {
        ...
        for (int i = 0; i < drawResultV.size(); i++) {
            ...
            if (!traffic.equals("0")) {
                plot.setSectionPaint(j, (Color)COLOR_CARDS.get(id));
                j++;
                continue;
            }
        }
    ...
    public JFreeChart drawLineScreenForLocal(String picTitle, String hTitle, String vTitle, Vector drawResultV, boolean isNeedLegend) {
        ...
        try {
            ...
            //System.out.println("serial 0 visiable"+renderer.getSeriesVisible(1));
    ...
    public void setInOutSeriesPaint(AbstractRenderer render) {
        ...
        if (this.firstSeriesName.equals(this.IN_NAME) || this.firstSeriesName.equals(this.IN_NAME_ENGLISH)) {
            renderer.setSeriesPaint(1, DrawUtil.COLOR_IN); //进站颜色
            renderer.setSeriesPaint(2, this.COLOR_OUT); //出站颜色
        } else {
            renderer.setSeriesPaint(1, this.COLOR_OUT); //出站颜色
            renderer.setSeriesPaint(2, this.COLOR_IN); //进站颜色
        }
