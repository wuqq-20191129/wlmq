...
public class NtpSynDao {

    static Logger logger = Logger.getLogger(NtpSynDao.class);
    ...
    public Vector queryStatuses() throws Exception {
        ...
        try {
            ...
            strSql = "select A.status_date,A.ip,A.status,A.remark,A.ip_source,A.status_date_syn,A.diff,B.name,"
                    + "B.type,B.ip_value "
                    + "from mtr_current_ntpsyn_info A,mtr_device_code B "
    ...
    public void addStatus(NtpSynVo vo) throws Exception {
        ...
        try {
            ...
            sql = "update mtr_current_ntpsyn_info set status_date=to_date(?,'yyyy-mm-dd hh24:mi:ss'), status=?,remark=? ,"
                    + "ip_source=?,status_date_syn=to_date(?,'yyyy-mm-dd hh24:mi:ss') ,diff=? where ip=? ";

            n = dbHelper.executeUpdate(sql, values);
            if (n != 1) {
                sql = "insert into mtr_current_ntpsyn_info(status_date,ip,status,remark,ip_source,status_date_syn,diff) values(to_date(?,'yyyy-mm-dd hh24:mi:ss'),?,?,?,?,to_date(?,'yyyy-mm-dd hh24:mi:ss'),?) ";
                n = dbHelper.executeUpdate(sql, values1);
            }
            sql = "insert into mtr_his_ntpsyn_info(status_date,ip,status,remark,ip_source,status_date_syn,diff) values(to_date(?,'yyyy-mm-dd hh24:mi:ss'),?,?,?,?,to_date(?,'yyyy-mm-dd hh24:mi:ss'),?) ";
            n = dbHelper.executeUpdate(sql, values1);
            dbHelper.commitTran();
        } catch (Exception e) {
