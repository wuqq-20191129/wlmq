...
function clickResultRowForColors(formName,object,detailFrom){
    ...
    if (oldLine != ""){
        var oldObject=document.getElementById(oldLine);
        oldObject.style.background=document.getElementById("rowColor").value;
    }
    //设置当前行00

    setCurrentLine(formName,headEventObject.id);
    //改变背景色
    document.getElementById("rowColor").value=headEventObject.style.background;
    headEventObject.style.background="#97CBFF";
...
function overResultRowForColors(formName,object){
    var frm = document.forms[formName];
    if(frm ==null)
        return;
    var rowSelected=frm.all["rowSelected"];
...
function outResultRowForColors(formName,object){
    var frm = document.forms[formName];
    if(frm == null)
        return;
    var rowSelected=frm.all["rowSelected"];
...
function JM_PowerListByIDs(clearStart,DataTable,colNum){
    ...
    for (i=0;i<trObject.length;i++){
        ...
        if(trObject[i].id=='ignore'){
            var cssText = "top: expression(this.offsetParent.scrollTop);";
...
function setcontrolDefaultValue(formName,controlName,defValue){
    ...
    if(ob == null)
        return;
    var lineNameOb = frm.all['_lineName'];
    var stationNameOb = frm.all['_stationName'];
    var stationCVOb =  frm.all['_stationCommonVariable'];
    var mainCardNameOb = frm.all['_mainCardName'];
    var subCardNameOb = frm.all['_subCardName'];
    var subCardCVOb =frm.all['_subCardCommonVariable'];
...
function setLineCardNames(formName,lineName,stationName,stationCV,mainCardName,subCardName,subCardCV){
    ...
    //alert("formName="+formName+" controlNames="+controlNames);
    var frm = document.forms[formName];
    if(frm.all['_lineName'] !=null && lineName !='')
        frm.all['_lineName'].value=lineName;
    if(frm.all['_stationName'] !=null && stationName !='')
        frm.all['_stationName'].value=stationName;
    if(frm.all['_mainCardName'] !=null && mainCardName !='')
        frm.all['_mainCardName'].value=mainCardName;
    if(frm.all['_subCardName'] !=null && subCardName !='')
        frm.all['_subCardName'].value=subCardName;
    if(frm.all['_stationCommonVariable'] !=null && stationCV !='')
        frm.all['_stationCommonVariable'].value=stationCV;
    if(frm.all['_subCardCommonVariable'] !=null && subCardCV !='')
        frm.all['_subCardCommonVariable'].value=subCardCV;
}
