...
//        if(request.getParameter("command")==null || request.getParameter("command").equals("list")){
//            request.getSession().setAttribute(NAME_BUFFER, null);
//        }
...
//        String msg = "当前共" + vo.getTotalRecords() + "条记录，共" + vo.getTotal() + "页，第" + vo.getCurrent() + "页" + ",记录数" + vo.getCurrentRecords() + "条";
    ...
    public void seperateResults(HttpServletRequest request, ModelAndView mv, OperationResult opResult) {
        ...
        if (command != null && (command.equals(CommandConstant.COMMAND_QUERY)
                ...
                || command.equals(CommandConstant.COMMAND_IMPORT)
                || command.equals(CommandConstant.COMMAND_AUDIT)) {
    ...
    private PageVo getNextPageControl(HttpServletRequest request) {
        ...
        if (command.equals(CommandConstant.COMMAND_QUERY) || command.equals(CommandConstant.COMMAND_DELETE) 
                || command.equals(CommandConstant.COMMAND_CHECK1) || command.equals(CommandConstant.COMMAND_CLONE)) {
            vo.setBackEnd("1");
