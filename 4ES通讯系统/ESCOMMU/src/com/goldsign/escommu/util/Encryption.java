...
public class Encryption{
    ...
    /**
     * uniEncrypt(String aPSW) is a uni-directional encryption from a string to another one. 
     * the encrypted string can not be decrypted to original any more.
     */   
    public String uniEncrypt(String aPSW) {
        ...
        try {
            ...
            DateHelper.screenPrintForEx("Error No:6093|PSWHistory uniEncrypt(String aPSW) err:" + e);
        }
        messageDigest.reset();
    ...
    /** biEncrypt(String aKey,String aWord) is a bi-directional encryption function for Encrypt a string with specific word.
     * encrypted string can be once again decrypted to original with the same word using biDecrypt(String aKey,String aWord).
     */ 
    ...
    /** biDecrypt(String aKey, String aWord) can decrypted the string which is encrypted by
     *  biEncrypt(String aKey, String aWord) to its original
     */
     ...
     *
     * You can use the toHexString() to transform a byte[] into a Hex String first and then use
     * this member function to transform back to a byte[].
     *
     ...
     *
     * You can use this memeber function to transform a byte[] into a Hex String first and then use
     * the toByteArray to transform back to a byte[].
     *
    ...
    public static void main(String[] args){
        ...
        String eStr = e.biEncrypt(CommuConstant.ENCRYPT_KEY, str);
        String bStr = e.biDecrypt(CommuConstant.ENCRYPT_KEY, eStr);
        System.out.println(eStr + ":" + bStr);
    }
}