...
public class CommuXMLHandler extends DefaultHandler {
    ...
    public void startElement(String namespaceURI, String localName, String qName, Attributes attr) throws SAXException {
        ...
        if (localName.equals(ConfigTagConstant.Common_TAG)) {
            this.inCommon = true;
        }
        if (localName.equals(ConfigTagConstant.FtpPath_TAG)) {
            ftpPaths.put(attr.getValue(id_ATTR), attr.getValue(value_ATTR));
        }
        if (localName.equals(ConfigTagConstant.Message_TAG)) {
            messages.put(attr.getValue(id_ATTR), attr.getValue(value_ATTR));
        }

        if (localName.equals(ConfigTagConstant.DataConnectionPool_TAG)) {
            this.inDataConnectionPool = true;
        }

        if (localName.equals(ConfigTagConstant.CommunicationThreadPool_TAG)) {
            this.inCommunicationThreadPool = true;
        }
        if (localName.equals(ConfigTagConstant.ConnectionPoolListenerThread_TAG)) {
    ...
    public void endElement(String namespaceURI, String localName, String qName) throws SAXException {
        ...
        if (localName.equals(ConfigTagConstant.Config_TAG)) {
            config.put(ConfigTagConstant.FtpPaths_TAG, ftpPaths);
            config.put(ConfigTagConstant.Messages_TAG, messages);
            config.put(ConfigTagConstant.DataConnectionPool_TAG, this.dataConnectionPool);
            config.put(ConfigTagConstant.CommunicationThreadPool_TAG, this.communicationThreadPool);
            config.put(ConfigTagConstant.ConnectionPoolListenerThread_TAG, this.connectionPoolListenerThread);
