...
public class BaseController {
    ...
    // @Autowired
    //  protected PrmVersionMapper prmVersionMapper;
    ...
    public void baseHandler(
        ...
        try {
            ...
            if (moduleID != null) {
                ...
                // this.saveCurrentVersionInfo(request, modelView);
                if (mv != null) {
//                    把mv通过btn转换为
    ...
    private boolean isNeedSaveQueryCondition(HttpServletRequest request) {
        ...
        if (!command.equals(CommandConstant.COMMAND_QUERY) && !command.startsWith(CommandConstant.COMMAND_QUERY)
                ...
                //                && !command.equals("next")
                //                && !command.equals("nextEnd")   // modify by lcuk 20171020 翻页保存查询条件
                ) {
            return false;
        }
        return true;
...
//            }
    ...
    /*del
    public void saveCurrentVersionInfo(HttpServletRequest request, ModelAndView modelView) throws Exception {
        ...
        PrmVersion vo = this.getCurrentVersionInfo(paramTypeID);
        if (vo != null) {
            modelView.addObject("CurrentVersionInformation", vo);
        }

    }
     */
 /*del
    private PrmVersion getCurrentVersionInfo(String paramTypeID) {
        List<PrmVersion> vers = this.prmVersionMapper.selectPrmVersionForCurrent(paramTypeID);
    ...
    protected boolean isUpdateOp(String command, String operType) {
        ...
        if (command != null) {
            // modify by lcuk 20171020 翻页保存查询条件
